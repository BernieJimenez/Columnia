...
Revisado: 2026-09-22. Esta es la única cola operativa derivada de la
...
Avance RV16 del 2026-09-22: el motor separa dieciséis áreas, entre ellas
validación de workspace/proyecto (`dataset/project_validation.rs`), perfiles
de importación (`dataset/import_profile_validation.rs`), consultas
...
comparación, rutas seguras, perfiles numéricos y lectura paginada
(`dataset/snapshot_comparison.rs`, `dataset/file_validation.rs`,
`dataset/numeric_profile.rs` y `dataset/page_reader.rs`). El lector nuevo
...
El corte anterior registró 494 pruebas aprobadas y 5 ignoradas. En esta
extracción pasan `cargo fmt --manifest-path src-tauri/Cargo.toml --all` y `cargo check --manifest-path src-tauri/Cargo.toml --lib`; la suite no se
volvió a ejecutar. RV16 sigue abierta.
...
| RV16 | **Modularización gradual del motor.** Extraer una responsabilidad de `dataset.rs` por cambio con contratos estables, paridad conductual y sin reescritura general. | Mantenimiento | Al tocar el área por RV02–RV05 o RV12 | **En curso — dieciséis módulos extraídos.** Validación de workspace, perfiles de importación, consultas, comparación, perfiles numéricos/categóricos/temporales, contratos y evaluación de calidad, recetas eager/lazy/source-backed, encabezados delimitados, seguridad CSV/Bundle, rutas seguras y lectura paginada viven en módulos internos separados. El módulo nuevo `dataset/page_reader.rs` concentra la paginación en memoria, lectura Parquet con slice pushdown y lectura source-backed CSV/TSV/TXT; preserva límites, cancelación y errores de fuente modificada. No cambia la API Tauri. El corte previo pasó 494 pruebas con 5 ignoradas; este corte pasa `cargo fmt --manifest-path src-tauri/Cargo.toml --all` y `cargo check --manifest-path src-tauri/Cargo.toml --lib`, sin volver a ejecutar la suite. Continúa como trabajo gradual. |

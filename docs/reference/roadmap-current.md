...
consultas (`dataset/local_query.rs`), estadísticas numéricas
...
la agregación temporal. `dataset.rs` conserva la API y los contratos. La suite
Rust completa pasa 494 pruebas; quedan 5 ignoradas (dos benchmarks opt-in y tres
integraciones ODBC externas). `cargo check --tests` también pasa. La matriz
incremental del 2026-09-21 pasó 16/16. RV16 sigue abierta para extracciones
graduales futuras.

...
Avance RV16 del 2026-09-22: el motor separa veinte áreas, entre ellas
...
| RV16 | **Modularización gradual del motor.** Extraer una responsabilidad de `dataset.rs` por cambio con contratos estables, paridad conductual y sin reescritura general. | Mantenimiento | Al tocar el área por RV02–RV05 o RV12 | **En curso — veinte módulos extraídos.** Validación de workspace, perfiles de importación, consultas, comparación, perfiles numéricos/categóricos/temporales, contratos y evaluación de calidad, recetas eager/lazy/source-backed, encabezados delimitados, seguridad CSV/Bundle, rutas seguras y lectura paginada viven en módulos internos separados. El módulo nuevo `dataset/page_reader.rs` concentra la paginación en memoria, lectura Parquet con slice pushdown y lectura source-backed CSV/TSV/TXT; preserva límites, cancelación y errores de fuente modificada. `dataset/import_schema_preview.rs` contiene el preflight del candidato detrás del mismo comando Tauri; `dataset/history.rs` reúne `HistoryManager` y sus contratos serializados, reexportados por `dataset.rs` con los mismos nombres JSON. `dataset/import_source_inspection.rs` agrupa selector, drag/drop, hojas Excel y revisión de encabezados; `dataset/import_loading.rs` contiene carga final y descarte con publicación atómica. Los comandos conservan nombres y parámetros Tauri. El corte previo pasó 494 pruebas con 5 ignoradas; este corte pasa `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check` y `cargo check --manifest-path src-tauri/Cargo.toml --lib`, sin volver a ejecutar la suite. Continúa como trabajo gradual. |

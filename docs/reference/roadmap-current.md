...
Avance RV16 del 2026-09-22: el motor ya separa siete áreas: validaciones de
...
consultas (`dataset/local_query.rs`), estadísticas numéricas y correlaciones
numéricas eager/source-backed (`dataset/numeric_profile.rs`), resúmenes
categóricos (`dataset/categorical_profile.rs`) y perfiles de tendencia temporal
(`dataset/temporal_profile.rs`). Este último comparte el cálculo de periodos con
la agregación temporal. `dataset.rs` conserva la API y los contratos, incluidos
los límites de muestreo. La suite Rust completa pasa 494 pruebas; quedan 5

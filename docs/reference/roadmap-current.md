...
| --- | --- | --- | --- | --- |
| RV01 | **Flujo contextual y estado central.** Una sola acción primaria por estado; visitar una fase no la completa; carga, perfil, plan, aplicación, validación y entrega rechazan resultados obsoletos o dobles ejecuciones. | Producto + frontend | Ninguna | **Parcial.** Revisar se completa con su acción contextual; Preparar y Entregar derivan «Hecho» de una aplicación/exportación exitosa y retirarlo ante un nuevo intento. El estado de finalización ya sigue resultados reales; queda centralizar la coordinación y cobertura de operaciones en curso. |
...
| ID | Resultado y criterio de cierre | Responsable | Dependencia | Estado / evidencia |
| --- | --- | --- | --- | --- |
| RV12 | **Recursos y escala medibles.** Una matriz varía ancho, cardinalidad, texto y tamaño; mide RAM, disco, tiempo, cancelación y limpieza. Los avisos solo aparecen cuando cambian una decisión y no confunden estimación con reserva. | Rendimiento | Evidencia de RV07 | **Parcial.** `perf:benchmark` mide tres cancelaciones del export CSV source-backed tras observar escritura temporal, registrando latencia hasta el retorno, publicación parcial y limpieza. La corrida `standard` de 1 MiB fue una prueba abreviada del harness (2 transformaciones y 1 actualización de proyecto): registró máximo de 16,56 ms, sin publicación parcial y con limpieza confirmada, pero no satisface las repeticiones mínimas comparables. El contrato conserva incompletos los ocho cruces hasta repetirlos con al menos 3 transformaciones y 2 actualizaciones de proyecto. La cancelación medida cubre el motor nativo y excluye el despacho UI/IPC. |

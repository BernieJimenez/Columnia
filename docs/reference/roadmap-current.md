...
Avance RV16 del 2026-09-22: las validaciones del perfil y del workspace de
proyecto ya viven en `dataset/project_validation.rs`, con la API preservada.
El perfil de `tools/check.ps1` habilita el manifiesto solo durante
`cargo test --lib` y restaura la variable al acabar. `tools/check-incremental-matrix.mjs`
pasa la opción a cada proceso de regresión nativa en Windows MSVC. `cargo test --lib`
pasa 494 pruebas y deja 5 ignoradas porque requieren servicios/drivers externos.
`npm run incremental:check` pasa sus 16 regresiones nativas enlazadas. También se corrigen los fixtures de migración v6–v13 y se conserva el orden de
filas activas antes de filas exclusivas comparadas en FULL JOIN. RV16 sigue
...
| RV16 | **Modularización gradual del motor.** Extraer una responsabilidad de `dataset.rs` por cambio con contratos estables, paridad conductual y sin reescritura general. | Mantenimiento | Al tocar el área por RV02–RV05 o RV12 | **En curso — primer módulo extraído el 2026-09-21; suite Rust operativa desde el 2026-09-22.** Las validaciones de workspace y perfil viven en `dataset/project_validation.rs`; `cargo test --lib` pasa 494 pruebas y 5 quedan ignoradas por servicios/drivers externos; `npm run incremental:check` pasa 16/16. Continúa como trabajo gradual al tocar otras áreas. |

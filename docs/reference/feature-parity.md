...
| Visualizaciones | Completitud, outliers, patrones de nulos, formatos, grupos, calendario y correlaciones, con tablas equivalentes; tendencia temporal por filas/porcentaje o suma/promedio de una columna numérica, agrupada por día, mes o año | Implementada | Ampliar interacciones |
| Salidas | CSV, JSON, Parquet, SQL, Excel, SQLite, bundle ZIP auditable y entrega ODBC a PostgreSQL/MySQL/SQL Server con transmisión source-backed y desde snapshots Parquet durables compatibles | Implementada | Políticas de esquema remoto más ricas y streaming para fuentes aún incompatibles |
...
DuckDB lee ambos lados desde disco y publica el resultado como una revisión
reversible; un historial degradado o un cursor incompatible mantiene el fallback
eager explícito.

La paginación de conflictos, su resolución y la consolidación reutilizan ese
mismo snapshot durable cuando el cursor actual es compatible. Las lecturas
validan que el snapshot no haya cambiado durante el recorrido y vuelven al
camino eager si el historial no puede ofrecer una fuente estable.
...
La entrega remota ODBC también reutiliza el snapshot Parquet durable del cursor
actual cuando el dataset ya está materializado. DuckDB transmite las filas por
bloques a PostgreSQL, MySQL o SQL Server, conservando calidad, privacidad,
...
La limpieza `Apartar tipos incompatibles` también puede inferir el tipo
dominante y apartar valores inválidos directamente en DuckDB cuando el activo
conserva una fuente delimitada o snapshot Parquet compatible. El cálculo usa
la fuente efectiva del cursor, conserva identificadores con ceros iniciales,
cuenta filas/celdas modificadas y publica un snapshot reversible; si el formato
o la expresión no son compatibles, vuelve al camino eager.

La corrección `Corregir codificación UTF-8` puede reparar en DuckDB las
secuencias mojibake inequívocas que contienen únicamente caracteres seguros.
Antes de publicar, la operación detecta valores ambiguos o mezclados y vuelve
al camino eager completo, evitando cambios parciales respecto a la función de

...
con una frontera de recursos de DuckDB y publicación atómica. Desde v0.69.0,
la misma ruta incorpora JSON, conserva el orden de origen y mantiene la
validación de cambios y el cleanup. Recetas, privacidad y operaciones que
...
no se habilita SQL arbitrario, tablas externas ni escritura. Si un JOIN es
compatible y el activo y la comparación tienen fuentes de disco válidas, la ruta
Polars también lo promueve a DuckDB aunque sea pequeño; así evita materializar
ambos datasets solo para ejecutar la unión. Esta promoción también cubre JOINs
grandes cuando existen fuentes de disco válidas; el `DataFrame` activo, la
...
1. **Paridad funcional:** el catálogo determinista completo de limpieza sugerida
   ya tiene acciones/replay equivalentes; todavía quedan la optimización global
   del plan y el análisis exploratorio

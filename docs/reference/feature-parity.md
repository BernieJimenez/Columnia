...
| Escala | Lazy/incremental para entradas grandes | Lazy para recetas compatibles, incluidos parseos explícitos `Ymd`/`Dmy`/`Mdy`; benchmark CLI validado hasta 256 MiB; perfiles Rayon persistentes; SQL local con Polars predeterminado o DuckDB opcional que reutiliza snapshots Parquet administrados del activo y la comparación y conserva fallback temporal, cancelación cooperativa, presupuesto de agregación y preflight de cardinalidad JOIN | Parcial | Ejecución incremental real fuera de RAM y presupuesto integral |

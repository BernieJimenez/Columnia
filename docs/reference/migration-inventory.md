...
| `drop_high_null_cols` | Acción directa de Preparar e importación de sesión | Reproducida en el fallback con el umbral estricto de DataPrep (>80% nulos) calculado sobre el perfil inicial, conservando al menos una columna utilizable aunque haya deduplicación previa |
| `drop_id_cols` | Acción directa de Preparar e importación de sesión | Reproducida en el fallback para columnas 100% únicas no numéricas ni temporales según el perfil inicial; una deduplicación previa no crea artificialmente una candidata y se conserva al menos una columna utilizable |
...
pipelines persistidos, y los normaliza a nombres canónicos. Cuando el usuario
usa una receta DataPrep como `--recipe` de `project-save`, esas operaciones se
reproducen antes de la transformación estructural y se validan junto al
dataset antes de escribir el proyecto. `parse_dates`

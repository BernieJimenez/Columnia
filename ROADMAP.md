...
- [x] **[T10-26] Formatear números, tamaños y tipos desde un único módulo**
  ...
  - **Esfuerzo:** bajo
  - **Depende de:** ninguna
  - **Cerrada:** 2026-09-23 — `src/format.ts` (decimales y porcentajes con el locale del sistema, bytes en unidades binarias, tipos del motor en español) sustituye los cinco formateadores y los `toFixed` visibles; antes convivían KB/KiB y los locales `es`, `es-DO` y del sistema. Las pruebas aceptan `.` o `,`; vitest 466/466 y E2E 26/26. Se conservan `toFixed` en coordenadas SVG y `aria-valuenow`, y la etiqueta GB del monitor de recursos.
...
| 2026-09-23 | 21 de 33 | Fase 2 en curso: cerradas hasta ahora T10-26 y anteriores de esta tanda. |

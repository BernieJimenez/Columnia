...
| 2026-09-20 | RV04: las tareas reutilizables guardan políticas versionadas para conversiones de tipo/fecha, asociadas a receta y esquema exacto. El slice conserva `review`; acciones todavía no implementadas se bloquean, y la revisión del esquema invalida la política. Si una receta repite una conversión de entrada, se omite la política derivada para no impedir guardar la tarea completa. Frontend focalizado (11/11), build y compilación Rust `--no-run` pasan; el test Rust focalizado quedó pendiente porque otro diagnóstico mantenía ocupado el lock de `target`. No completa la resolución de excepciones ni los conflictos, y no altera la aceptación nativa pendiente. | `src-tauri/src/dataset.rs`, `src-tauri/src/reusable_tasks.rs`, `src/bridge/reusable-task-contracts.ts`, `src/features/load/ReusableTaskPanel.tsx`, `src/features/load/reusableTaskExceptions.ts`, `docs/reference/roadmap-current.md` |

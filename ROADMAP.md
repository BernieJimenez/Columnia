...
- [ ] **RV16 — Modularización gradual del motor.** Extraer responsabilidades de
  ...
  - **Avance 2026-09-21:** RV16 cuenta con trece módulos extraídos desde
    ...
    source-backed en `dataset/recipe_source_projection.rs`, protección de fórmulas de texto en exportaciones CSV/Bundle en `dataset/csv_formula_safety.rs`, resúmenes
    ...
    Tras este corte pasan `cargo fmt --all -- --check`, `cargo check --tests` y `cargo test --lib` con el harness Windows Common Controls v6; la suite Rust
    completa registra 494 aprobadas, 0 fallidas y 5 ignoradas (dos benchmarks
    opt-in y tres integraciones ODBC externas). La matriz incremental previa
    pasó 16/16. RV16 sigue abierta para extracciones graduales.
...
| 2026-09-21 | RV04: Abrir proyecto ahora consulta `projectOpen` durante el hash SHA-256, conteo DuckDB, lectura Parquet y copia/validación de snapshots del historial. La actualización de `last_opened_at` y la activación del dataset comparten un gate con cancelar; si la cancelación gana, ambos estados previos se conservan. «Cancelar apertura» solo aparece en la apertura normal; Restaurar conserva su ruta transaccional. `cargo fmt --check`, `cargo check --lib`, `npm run build`, `npm run ipc:check`, `npm run docs:check` y `git diff --check` pasan; el verificador documental aprobó sus 4 pruebas internas y no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/projects.rs`, `src/bridge/delivery-contracts.ts`, `src/features/projects/useProjectsController.ts`, `src/features/projects/ProjectsPanel.tsx`, `src/App.tsx`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-21 | RV04: restaurar versión comparte `projectOpen`, prepara y valida el candidato antes de persistir, y activa el dataset bajo el mismo gate que confirma el commit del catálogo. Cancelar antes de ese gate conserva la versión guardada y la sesión activa; la interfaz ofrece «Cancelar restauración». Pasan `cargo fmt --check`, `cargo check --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/projects.rs`, `src/features/projects/useProjectsController.ts`, `src/features/projects/ProjectsPanel.tsx`, `src/App.tsx`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-21 | RV04: guardar manualmente y autoguardar comparten `projectSave`; la materialización source-backed, el hash y las copias del snapshot/historial observan el token, y el catálogo se confirma bajo un gate de cancelación. La generación staged se elimina si cancelar gana antes del commit; si el commit gana, guardar termina sin dejar referencias a archivos borrados. El escritor Parquet, `sync_all` y la transacción SQLite siguen siendo síncronos y verifican el token a continuación o al entrar al gate. La UI permite «Cancelar guardado» y «Cancelar autoguardado». `cargo fmt --check`, `cargo check --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check` pasan; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/projects.rs`, `src/bridge/delivery-contracts.ts`, `src/features/projects/useProjectsController.ts`, `src/features/projects/ProjectsPanel.tsx`, `src/App.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: eliminar proyecto usa `projectDelete` al reunir y validar rutas de snapshots y versiones; si cancelar gana antes del gate, no cambia el catálogo ni se borran archivos. El gate cubre el DELETE SQLite y la limpieza para que una cancelación tardía espere a que la operación termine; SQLite y `remove_dir_all` siguen siendo síncronos, y un error de limpieza posterior al commit se comunica como proyecto eliminado con archivos pendientes. La UI cierra la confirmación al iniciar y ofrece «Cancelar eliminación». Pasan `cargo fmt --check`, `cargo check --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/projects.rs`, `src/bridge/delivery-contracts.ts`, `src/features/projects/projectModel.ts`, `src/features/projects/useProjectsController.ts`, `src/features/projects/ProjectsPanel.tsx`, `src/App.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: la lista de proyectos consulta cancelación por fila y al medir los bytes de snapshots e historial; una generación de solicitud frontend descarta respuestas obsoletas. La UI ofrece «Cancelar carga» y reintento. `get_recovery_candidate` sigue siendo una consulta separada de una fila sin token. `cargo fmt --check`, `cargo check --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check` pasan; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/projects.rs`, `src/bridge/delivery-contracts.ts`, `src/features/projects/projectModel.ts`, `src/features/projects/useProjectsController.ts`, `src/features/projects/ProjectsPanel.tsx`, `src/App.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: `list_projects` devuelve el catálogo y el candidato de recuperación en una sola respuesta y transacción de lectura bajo `projectCatalog`. La migración SQLite v15 crea el índice parcial; la lista y la consulta del candidato comparten la misma vista de lectura, y la consulta indexada comprueba cancelación antes y después de su tramo SQLite síncrono. Se elimina la invocación separada `get_recovery_candidate`; se actualizan el bridge, los mocks frontend/E2E, el smoke nativo y el inventario IPC. Pasan `cargo fmt --check`, `cargo check --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/projects.rs`, `src-tauri/src/lib.rs`, `src/bridge/project-contracts.ts`, `src/bridge/projects.ts`, `src/features/projects/useProjectsController.ts`, `src/features/projects/useProjectsController.test.tsx`, `src/App.test.tsx`, `e2e/`, `tools/probe-webview2-projects.mjs`, `tools/check-ipc-inventory.mjs`, `docs/reference/ipc-inventory.json`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: la vista previa `preview_delimited_header_review` comparte la generación `load` de la selección. Rust comprueba el token alrededor de la lectura acotada a 64 KiB, la detección de delimitador, el staging temporal, los dos parseos de muestra y la revalidación de tamaño; «Cancelar» también envía `cancel_operation("load")` antes de descartar la selección. Lectura y parseo siguen siendo síncronos, pero al retornar se descarta cualquier resultado cancelado. Pasan `cargo fmt --check`, `cargo check --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src/App.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: la detección del delimitador lee la muestra de 64 KiB en bloques de 8 KiB y revisa cancelación entre lecturas y entre los cuatro candidatos de delimitador. Tanto `preview_delimited_header_review` como la carga completa CSV/TSV/TXT propagan la generación `load`; la lectura Polars comienza solo tras terminar la detección. Pasan `cargo fmt --check`, `cargo check --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: el catálogo local de tareas reutilizables usa una generación cancelable, revisada entre filas SQLite y deserializaciones. El panel ofrece «Cancelar carga» y reintento, conserva el catálogo anterior al cancelar una actualización y descarta respuestas obsoletas. La lectura de una fila SQLite y su deserialización siguen siendo síncronas. Pasan `cargo fmt`, `cargo check --lib`, `npm run build`, `npm run ipc:check` y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/reusable_tasks.rs`, `src-tauri/src/dataset.rs`, `src/bridge/delivery-contracts.ts`, `src/features/load/useReusableTasks.ts`, `src/features/load/ReusableTaskPanel.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
...
| 2026-09-21 | RV01/RV04: `list_project_versions` aplica `LIMIT 5` en SQLite y solo deserializa las versiones recientes, incluso si el catálogo contiene filas sobrantes. Pasan `cargo fmt --check`, `cargo check --lib` (58 avisos dead-code existentes), `npm run build`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/projects.rs`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: recuperación de generaciones consulta referencias SQLite sin convertir errores en un conjunto vacío; si no puede leerlas, omite la limpieza. La inicialización y el recorrido/eliminación de huérfanos revisan cancelación. `list_project_versions` usa `projectVersions`, recorre hasta cinco entradas con cancelación y Proyectos ofrece cancelar/reintentar. `cargo fmt --check`, `cargo check --lib`, `npm run build`, `npm run ipc:check` y `git diff --check` pasan; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/project_recovery.rs`, `src-tauri/src/projects.rs`, `src/bridge/delivery-contracts.ts`, `src/features/projects/projectModel.ts`, `src/features/projects/useProjectsController.ts`, `src/features/projects/ProjectsPanel.tsx`, `src/App.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV01/RV04: inicio, cancelación y commit de la carga comparten `load_commit_lock`. La inspección publica selección/hojas solo bajo una generación vigente; tras crear el historial, la carga comprueba token e ID de selección y sustituye dataset, comparación y selección bajo el gate. Cancelar durante la creación síncrona del historial descarta el candidato y conserva la sesión anterior; si el commit entra primero, cancelar espera a esa publicación. Pasan `cargo fmt --check`, `cargo check --lib` (58 avisos dead-code existentes), `npm run build`, `npm run ipc:check` y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/tests.rs`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: `test_database_connection` ya no bloquea el executor Tauri: ODBC se ejecuta en `spawn_blocking` bajo el token independiente `databaseConnection`, con comprobaciones antes/después de inicializar el entorno, abrir la conexión y ejecutar `SELECT 1`. El driver no interrumpe una llamada activa; si cancelar llega durante ella, el resultado se descarta al volver. El comando independiente no tiene un consumidor actual en la interfaz. Pasan `cargo fmt --check`, `cargo check --lib` (58 avisos dead-code existentes), `npm run build`, `npm run ipc:check` y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/remote_databases.rs`, `src-tauri/src/dataset.rs`, `src/bridge/delivery-contracts.ts`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: la comprobación de actualizaciones de red usa el token `updateCheck` y el panel permite cancelarla. `tokio::select!` descarta la consulta activa; la cancelación y la publicación comparten un gate con generación para que solo una gane. Pasan `cargo fmt --check`, `cargo check --lib` (58 avisos dead-code existentes), `npm run build`, `npm run ipc:check` (84 producción, 4 debug, 69 estructuras), el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/updater.rs`, `src-tauri/src/dataset.rs`, `src/bridge/delivery-contracts.ts`, `src/components/UpdatePanel.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: las exportaciones source-backed Parquet y JSON usan el monitor de cancelación de DuckDB durante `COPY`; la copia al temporal de publicación comprueba el token cada 64 KiB. `sync_all` permanece síncrono y el destino solo se publica al completar la copia. Pasan `cargo fmt --check`, `cargo check --lib` (58 avisos dead-code existentes), `npm run build`, `npm run ipc:check` (84 producción, 4 debug, 69 estructuras), el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/duckdb_query.rs`, `src-tauri/src/dataset.rs`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: el hash SHA-256 del CSV temporal de Bundle comprueba cancelación al iniciar, entre lecturas de 64 KiB y después de rebobinar; se aplica a las rutas eager y source-backed. El Bundle incompleto no se publica y el conteo de bytes rechaza desbordamientos. Pasan `cargo fmt --check`, `cargo check --lib` (58 avisos dead-code existentes), `npm run build`, `npm run ipc:check` (84 producción, 4 debug, 69 estructuras), el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: la protección eager de privacidad y las exportaciones CSV/JSON/Parquet observan cancelación. Los escritores Polars procesan bloques de 8.192 filas; CSV neutraliza fórmulas por bloque, JSON combina bloques en un solo arreglo y Parquet usa escritura batched. El Bundle eager utiliza el mismo escritor CSV cancelable; el archivo de destino solo se publica tras completar la exportación. El cierre del escritor y `sync_all` siguen síncronos. Pasan `cargo fmt --check`, `cargo check --lib` (58 avisos dead-code existentes), `npm run build`, `npm run ipc:check` (84 producción, 4 debug, 69 estructuras), el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: la reconciliación de generaciones huérfanas consulta cancelación al enumerar y validar entradas, y antes/después de cada `std::fs::remove_dir_all` seguro. La llamada recursiva sigue siendo síncrona: una cancelación durante ese syscall se reconoce al regresar. El borrado explícito continúa bajo el gate si el commit del catálogo ya ocurrió. Pasan `cargo fmt --check`, `cargo check --lib` (58 avisos dead-code existentes), el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/project_recovery.rs`, `CONTEXTO.md`, `ROADMAP.md`, `docs/reference/roadmap-current.md` |

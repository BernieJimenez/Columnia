...
- [ ] Importar sesiones guardadas de DataPrep: dataset/origen, hoja, etapa,
  ...
  publicar el proyecto; ahora reproduce `impute_categorical` cuando falta el
  snapshot y la semántica es determinista; todavía no restaura resultados de análisis originales,
...
| 2026-08-29 | M1 reproduce durante el fallback a la fuente la operación DataPrep `impute_categorical` cuando su semántica fija permite hacerlo sin inventar parámetros; los snapshots compatibles siguen teniendo prioridad y no se reaplican. | `src-tauri/src/projects.rs`, `src-tauri/src/dataset.rs`, `docs/reference/migration-inventory.md` |

...
- [ ] Importar sesiones guardadas de DataPrep: dataset/origen, hoja, etapa,
  ...
  publicar el proyecto; ahora reproduce `normalize_sentinels`,
  `impute_categorical` y `fix_encoding` cuando falta el snapshot y la semántica
  es determinista, en el orden fijo de limpieza; todavía no restaura resultados de análisis originales,
...
| 2026-08-29 | M1 reproduce durante el fallback a la fuente las operaciones DataPrep deterministas `normalize_sentinels`, `impute_categorical` y `fix_encoding`, en el orden fijo del registro y sin inventar parámetros; los snapshots compatibles siguen teniendo prioridad y no se reaplican. | `src-tauri/src/projects.rs`, `src-tauri/src/dataset.rs`, `docs/reference/migration-inventory.md` |

...
- Etapa actual: Fase I1 completa como prototipo vertical verificable, con las
  ...
  durante el análisis conjunto. En
  v0.161.0, las pruebas
  de resolución de conflictos y consolidación sobre snapshots Parquet durables
  verifican publicación reversible, orden y frame activo sin filas. En v0.160.0, la
...
El trabajo local sí avanza, pero varias casillas dependen de evidencia que no se
puede producir solo con cambios de código: RV07 requiere tres sesiones humanas
con datasets reales; RV09 necesita una persona que use lector de pantalla nativo;
RV10 requiere una instancia accesible de SQL Server; RV11 necesita aprobación
jurídica y prueba de instalación en VM limpia. RV08 depende de los fallos de
RV07, RV14 de la herramienta BI elegida y RV15 solo se activa si la beta confirma
demanda repetida. RV01–RV06 conservan aceptación con flujos/datos reales pendiente
donde así lo marca la cola. Mientras se obtienen esos insumos, el avance local
continúa por RV16 en cortes graduales con comandos estables. Estado detallado y
siguiente evidencia requerida: [`docs/reference/roadmap-current.md`](docs/reference/roadmap-current.md).
...
- [ ] **RV06 — Interfaz compacta y accesible.** Reducir jerga y repetición,
  ...
  - **Avance local: 2026-09-22.** En Preparar, el último resultado y los controles
    Deshacer/Rehacer permanecen a la vista; la lista de cambios inicia plegada y
    se abre mediante su resumen nativo accesible, reduciendo altura sin ocultar
    acciones ni el resultado reciente. La aceptación nativa con lector de pantalla
    continúa pendiente.
...
- [ ] **RV16 — Modularización gradual del motor.** Extraer responsabilidades de
  ...
  - **Avance 2026-09-22:** RV16 cuenta con veintidós módulos que reúnen veinticuatro responsabilidades extraídas desde
    ...
    en `dataset/recipe_engine.rs`, planificación/ejecución source-backed en
    `dataset/recipe_source_projection.rs`, protección de fórmulas en
    `dataset/csv_formula_safety.rs`, revisión de encabezados en
    ...
    `get_dataset_page`, `dataset/conflict_reader.rs`, que orquesta `get_dataset_conflict_page`, y `dataset/history.rs`, que reúne `HistoryManager` con
...
| 2026-09-15 | RV01: App ahora liga selección/inspección, comparación, paginación de conflictos, consolidación, unión y exportación a secuencias y revisión del dataset, y descarta clics repetidos mientras la operación está pendiente; una regresión de exportación confirma una sola llamada al bridge. La aceptación nativa y la cancelación integral siguen abiertas. | `src/App.tsx`, `src/App.test.tsx`, `docs/reference/roadmap-current.md` |
| 2026-09-15 | RV04: la acción principal de Entregar serializa la validación de calidad y descarta su resultado si cambian reglas, formato, protección, destino o dataset mientras espera; una regresión de DeliveryPhase confirma una sola validación ante dos clics síncronos. La cancelación uniforme de preparación y la aceptación nativa siguen abiertas. | `src/features/delivery/DeliveryPhase.tsx`, `src/features/delivery/DeliveryPhase.test.tsx`, `docs/reference/roadmap-current.md` |
| 2026-09-19 | RV01/RV04: Preparar añade Cancelar único para mutaciones, recetas, deshacer y rehacer; tokens compartidos interrumpen DuckDB source-backed y bloquean publicaciones/cambios de cursor si la cancelación gana la carrera. Se agregan preparación cancelable de snapshots y regresiones para rollback de historial y limpieza source-backed→receta→deshacer/rehacer. La publicación síncrona de JOIN/consolidación en Review queda fuera de este token. Frontend (407 pruebas), build y E2E de accesibilidad (3/3) pasan; `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y compilación de pruebas Rust pasan. El ejecutable nativo termina antes del harness con `STATUS_ENTRYPOINT_NOT_FOUND`, así que la aceptación nativa permanece abierta; RV04 también conserva pendientes las decisiones reutilizables para excepciones. | `src-tauri/src/dataset.rs`, `src-tauri/src/duckdb_query.rs`, `src/features/prepare/usePrepareController.ts`, `docs/reference/roadmap-current.md` |
| 2026-09-19 | RV02/RV05/RV06: la validación del perfil CSV distingue cabecera presente y generada, guardar una tarea la deja lista para la próxima importación y la comparación de revisiones queda plegada para reducir controles visibles. App/UI cubren esos recorridos; el E2E de accesibilidad comprueba foco visible también a 320 CSS px. La auditoría consolidada sigue siendo el único origen de hallazgos; RV04 recomienda reutilizar decisiones solo con esquema exacto y conservar revisión ante cambios. | `src-tauri/src/dataset.rs`, `src-tauri/src/reusable_tasks.rs`, `src/features/load/useReusableTasks.ts`, `src/features/prepare/PreparePhase.tsx`, `e2e/workflow-accessibility.spec.ts`, `docs/reference/roadmap-current.md` |
| 2026-09-20 | RV04: las tareas reutilizables guardan y ejecutan políticas versionadas de conversión de tipo/fecha, ligadas a receta y esquema exacto. `review` mantiene el error estricto; `nullify` nulifica solo valores inválidos y `excludeRow` retira filas inválidas con conteo, preservando nulos reales. La ruta source-backed prepara el candidato dentro del límite de materialización y publica de forma reversible tras completar; esquema, receta o política incompatibles fallan antes de publicar. `npm test` (416/416), build, IPC, `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y cinco pruebas Rust focalizadas de política pasan. El filtro Rust amplio de recetas quedó en 37/38: `source_backed_projection_recipe_writes_parquet_without_materializing_rows` falla al comparar rutas con el prefijo extendido de Windows; el test no se modificó. El ejecutable de pruebas directo sigue dando `STATUS_ENTRYPOINT_NOT_FOUND` en esta estación, por lo que la aceptación nativa continúa pendiente. RV04 sigue parcial por decisiones de conflictos y cancelación en Review. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/tests.rs`, `src-tauri/src/reusable_tasks.rs`, `src/bridge/prepare.ts`, `src/bridge/reusable-task-contracts.ts`, `src/features/prepare/usePrepareController.ts`, `src/features/load/ReusableTaskPanel.tsx`, `src/features/load/reusableTaskExceptions.ts`, `src/App.tsx`, `docs/reference/roadmap-current.md` |
| 2026-09-20 | RV02: las convenciones CSV/TSV de fecha/número se ejecutan durante la importación, con conversión atómica por columna: datos mixtos o incompatibles conservan el texto; `unresolved` tampoco convierte. Se preservan opciones al confirmar un esquema nuevo y el backend rechaza una discrepancia entre perfil y argumentos. Archivos source-backed quedan en léxico por el límite seguro de memoria. Ocho pruebas Rust focalizadas pasan con manifiesto Common Controls v6 temporal; `npm test` (418/418), build, IPC y E2E (21/21) pasan. Los E2E de CSV/XLSX/Parquet simulan Tauri y verifican la UI; no procesan bytes reales. La aceptación integrada con las tres fuentes sigue pendiente. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/import_conventions.rs`, `src/bridge/datasets.ts`, `src/features/load/loadModel.ts`, `src/features/load/LoadPhase.tsx`, `src/App.tsx`, `e2e/import-formats.spec.ts`, `ROADMAP.md`, `docs/reference/roadmap-current.md` |
| 2026-09-20 | RV04: JOIN y Consolidar en Review comparten exclusión mutua y cancelación mediante `reviewMutation`; la UI conserva la comparación, no publica resultados parciales y mantiene los controles bloqueados hasta que termina la llamada de cancelación, aunque la mutación ya haya resuelto. Las regresiones eager y source-backed cancelan dentro de `ReviewMutationCancellation::commit` y verifican comparación/historial intactos y limpieza de archivos; la ruta source-backed también comprueba staging y cancelación antes de COPY. Los lectores eager CSV/Parquet/Excel y el JOIN Polars monolítico solo observan cancelación antes/después de la llamada, por lo que la respuesta puede esperar al parser/JOIN; el selector nativo de archivos es modal y no se puede interrumpir desde la interfaz mientras está abierto. Una lectura eager acotada usada como fallback source-backed también puede terminar antes de observar la cancelación. La publicación aborta si cancelar llega antes del commit; las operaciones source-backed de DuckDB (COPY/validación) sí interrumpen consultas activas mediante DuckDB interrupt. `npm test` (423/423), `npm run test:e2e` (21/21), `npm run build`, `npm run ipc:check` y `npm run docs:check` pasan. Estado Rust actual: `cargo fmt --check`, `cargo check --tests` y filtro `review_` 10/10 pasan; también pasan cancelación pre-COPY, interrupción DuckDB activa, consolidación snapshot-backed reversible y proyección/undo/redo con rutas canonicalizadas (1/1 cada una). La suite Rust completa pasó 486 pruebas (0 fallidas, 5 ignoradas) en el binario de pruebas previo al test adicional independiente de cancelación en el commit gate; no se volvió a ejecutar la suite completa tras añadir ese test. RV04 sigue parcial por conflictos, límites de interrupción eager y aceptación real/nativa pendiente. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/tests.rs`, `src-tauri/src/duckdb_query.rs`, `src/App.tsx`, `src/App.test.tsx`, `src/bridge/delivery-contracts.ts`, `src/features/review/ReviewPhase.tsx`, `src/features/review/joinModel.ts`, `docs/reference/roadmap-current.md` |
| 2026-09-20 | RV05: al preparar una tarea, la importación posterior con el perfil y esquema exactos aplica automáticamente reglas, formato, privacidad y receta como borrador, sin un segundo diálogo. La receta sigue requiriendo ejecución explícita; sin el perfil guardado o tras confirmar un esquema distinto se conserva la revisión. No se incorpora permiso de sobrescritura ni credenciales. App comprueba que no se llama la transformación hasta aplicarla y que un esquema distinto no instala ajustes inesperados. `npm test` (425/425), build y `npm run test:e2e` (21/21) pasan. La aceptación nativa con archivos reales sigue pendiente. | `src/App.tsx`, `src/App.test.tsx`, `src/features/load/ReusableTaskPanel.tsx`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-20 | RV06: en Preparar, el botón de avance queda secundario solo mientras haya una acción local habilitada para aplicar el plan o la receta, dejando una acción principal clara por estado; los colores forzados usan los colores del sistema. El E2E compara la jerarquía visual a 200 % con un plan habilitado, comprueba la asignación de ButtonFace/ButtonText y recorre el avance con teclado a 320 CSS px; `npm test` (425/425), `npm run test:e2e` (21/21) y build pasan. La comparación continúa plegada y el historial/deshacer visible. Sigue pendiente la aceptación nativa con lector de pantalla. | `src/App.tsx`, `src/workflow-styles.css`, `e2e/workflow-accessibility.spec.ts`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-20 | RV06: se corrige el trap de teclado del diálogo: los controles dentro de `details` cerrados quedan fuera del recorrido y el resumen se incluye; la regresión confirma que Tab llega a «Cargar archivo». RV02: `smoke:native-selectors` ahora exporta y recarga CSV, XLSX y Parquet reales mediante los diálogos de Windows, y verifica dos filas, esquema, valores y hoja XLSX sobre un dataset sintético. El smoke de ProjectsPanel también pasó con Playwright/WebView2. `npm test` (426/426), E2E (21/21), build, IPC, documentación y formato pasan. La aceptación con datos de trabajo y lector de pantalla sigue pendiente. | `src/components/ModalDialog.tsx`, `src/components/ModalDialog.test.tsx`, `tools/probe-webview2-native-selectors.mjs`, `tools/probe-webview2-projects.mjs`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-20 | RV04: la carga eager CSV/TSV/TXT y Parquet recopila con Polars Streaming en lotes cancelables; JSON/JSONL revisa cancelación entre registros; XLSX/XLSB inspecciona celdas en dos pasadas con filas acumuladas por bloques, mientras XLS/ODS conserva lectura monolítica. Review ejecuta JOIN eager por bloques cancelables, preserva el orden y agrega las filas derechas sin pareja; cada bloque vuelve a comparar contra el dataset completo y puede elevar el coste. Se agregan cuatro regresiones, pero solo `cargo check --tests` las compiló. `cargo test --manifest-path src-tauri/Cargo.toml` termina antes del harness con `STATUS_ENTRYPOINT_NOT_FOUND` (`0xc0000139`), incluso con manifiesto Common Controls v6 temporal. RV04 sigue parcial por comandos sin token cancelable, XLS/ODS, selector modal, medición sobre datos reales y aceptación nativa. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/tests.rs`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-20 | RV04: los fallbacks que materializan fuentes source-backed en JOIN, resolución/consolidación de Review, mutaciones/recetas de Preparar y exportación local/ODBC propagan cancelación a lectores CSV/TSV/TXT/Parquet por lotes y JSON/JSONL/NDJSON por registro. El dataset solo se marca materializado al terminar; JOIN carga su fallback dentro de `spawn_blocking`. `cargo fmt` y `cargo check` pasan; no se ejecutaron pruebas. Quedan comparación y otros comandos sin token, selector nativo modal, XLS/ODS, medición con datos reales y aceptación nativa. | `src-tauri/src/dataset.rs`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-20 | RV04: la comparación de datasets incorpora un token propio `datasetComparison` y un control Cancelar en Review. La copia Parquet se hace por bloques; CSV/TSV/TXT/JSON se convierten con DuckDB interruptible; XLSX/XLSB recorre y escribe por bloques cancelables; Parquet compara filas e índices temporales con comprobaciones entre bloques/registros. El resultado previo se conserva si cancelar gana el gate de publicación. El selector nativo modal, XLS/ODS, conteos/escrituras síncronos y la paginación de conflictos fallback siguen como límites abiertos. `cargo fmt`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build` y `git diff --check` pasan; no se ejecutaron pruebas. | `src-tauri/src/dataset.rs`, `src-tauri/src/duckdb_query.rs`, `src/App.tsx`, `src/bridge/delivery-contracts.ts`, `src/features/review/ReviewPhase.tsx`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-20 | RV04: la paginación de conflictos eager y source-backed comparte `datasetComparison`; Review permite cancelar la carga, interrumpe materialización y recorrido por bloques/registros, y conserva la página previa cuando la cancelación gana. La respuesta valida que la comparación siga activa y usa el gate de cancelación antes de publicarse. `cargo fmt`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, documentación y `git diff --check` pasan; no se ejecutaron pruebas. | `src-tauri/src/dataset.rs`, `src/App.tsx`, `src/features/review/ReviewPhase.tsx`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-20 | RV04: la comparación de archivos cuenta snapshots Parquet con DuckDB interruptible y escribe snapshots eager por bloques, con comprobaciones entre lotes; `finish` y `sync_all` permanecen síncronos. `cargo fmt`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, documentación y `git diff --check` pasan; no se ejecutaron pruebas. | `src-tauri/src/dataset.rs`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-20 | RV04: la validación local de reglas de calidad en Entregar usa el token `qualityValidation` y permite cancelar. Se descarta el resultado al cancelar; la materialización DuckDB, bloques Parquet y recorridos por filas comprueban el token, mientras que agregaciones vectorizadas solo pueden hacerlo al regresar. `cargo fmt`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, el checker documental y `git diff --check` pasan; no se ejecutaron pruebas. | `src-tauri/src/dataset.rs`, `src/bridge/delivery-contracts.ts`, `src/features/delivery/DeliveryPhase.tsx`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-20 | RV04: el preflight de compatibilidad ODBC en Entregar usa `databasePreflight`; materialización, protección de privacidad y análisis eager/source-backed observan cancelación y el exportador eager revisa filas durante el análisis de columnas. Las llamadas síncronas al driver ODBC descartan el resultado cuando regresan. `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, el checker documental y `git diff --check` pasan; no se ejecutaron pruebas. | `src-tauri/src/dataset.rs`, `src-tauri/src/remote_databases.rs`, `src/bridge/delivery-contracts.ts`, `src/features/delivery/DeliveryPhase.tsx`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-21 | RV04: la enumeración de hojas Excel pasa a `inspect_workbook_sheets` después de devolver la selección a Cargar. La interfaz permite cancelar con la generación `load`, invalida la respuesta y descarta la selección pendiente; el backend rechaza generaciones obsoletas antes de publicar hojas. `open_workbook_auto` y `sheet_names` son síncronos, así que la cancelación se observa antes y después de la llamada. `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check` pasan; no se ejecutaron pruebas. | `src-tauri/src/dataset.rs`, `src-tauri/src/lib.rs`, `src/bridge/datasets.ts`, `src/features/load/loadModel.ts`, `src/features/load/LoadPhase.tsx`, `src/App.tsx`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-21 | RV04: la paginación principal de Review usa el token `datasetPage`; la UI mantiene visible la página actual y permite cancelar. Polars Streaming interrumpe las lecturas por lotes de snapshots Parquet y fuentes delimitadas; el fallback de materialización y el armado de filas consultan la generación. `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check` pasan; no se ejecutaron pruebas. XLS/ODS conserva lectura monolítica. | `src-tauri/src/dataset.rs`, `src/App.tsx`, `src/bridge/delivery-contracts.ts`, `src/features/review/ReviewPhase.tsx`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-21 | RV04: en `.xls`/`.ods`, `worksheet_range` permanece síncrono; después de recibir el rango completo, el análisis, la inferencia y conversión de columnas y la preparación/escritura de snapshots comprueban cancelación por lotes, columnas y bloques, descartando resultados incompletos. `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check` pasan; no se ejecutaron pruebas. | `src-tauri/src/dataset.rs`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-21 | RV04: cancelar el perfil de calidad conserva el estado visible en Revisar y Preparar, confirma que no se publicó un resultado parcial y cambia la acción principal a reintento explícito; los intentos cancelados no se repiten automáticamente. `npm run build`, el checker documental y `git diff --check` pasan; no se ejecutaron pruebas. | `src/App.tsx`, `src/features/review/reviewModel.ts`, `src/features/review/ReviewPhase.tsx`, `src/features/prepare/PreparePhase.tsx`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-21 | RV04: Abrir proyecto ahora consulta `projectOpen` durante el hash SHA-256, conteo DuckDB, lectura Parquet y copia/validación de snapshots del historial. La actualización de `last_opened_at` y la activación del dataset comparten un gate con cancelar; si la cancelación gana, ambos estados previos se conservan. «Cancelar apertura» solo aparece en la apertura normal; Restaurar conserva su ruta transaccional. `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check`, `npm run docs:check` y `git diff --check` pasan; el verificador documental aprobó sus 4 pruebas internas y no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/projects.rs`, `src/bridge/delivery-contracts.ts`, `src/features/projects/useProjectsController.ts`, `src/features/projects/ProjectsPanel.tsx`, `src/App.tsx`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-21 | RV04: restaurar versión comparte `projectOpen`, prepara y valida el candidato antes de persistir, y activa el dataset bajo el mismo gate que confirma el commit del catálogo. Cancelar antes de ese gate conserva la versión guardada y la sesión activa; la interfaz ofrece «Cancelar restauración». Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/projects.rs`, `src/features/projects/useProjectsController.ts`, `src/features/projects/ProjectsPanel.tsx`, `src/App.tsx`, `docs/reference/roadmap-current.md`, `CONTEXTO.md` |
| 2026-09-21 | RV04: guardar manualmente y autoguardar comparten `projectSave`; la materialización source-backed, el hash y las copias del snapshot/historial observan el token, y el catálogo se confirma bajo un gate de cancelación. La generación staged se elimina si cancelar gana antes del commit; si el commit gana, guardar termina sin dejar referencias a archivos borrados. El escritor Parquet, `sync_all` y la transacción SQLite siguen siendo síncronos y verifican el token a continuación o al entrar al gate. La UI permite «Cancelar guardado» y «Cancelar autoguardado». `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check` pasan; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/projects.rs`, `src/bridge/delivery-contracts.ts`, `src/features/projects/useProjectsController.ts`, `src/features/projects/ProjectsPanel.tsx`, `src/App.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: eliminar proyecto usa `projectDelete` al reunir y validar rutas de snapshots y versiones; si cancelar gana antes del gate, no cambia el catálogo ni se borran archivos. El gate cubre el DELETE SQLite y la limpieza para que una cancelación tardía espere a que la operación termine; SQLite y `remove_dir_all` siguen siendo síncronos, y un error de limpieza posterior al commit se comunica como proyecto eliminado con archivos pendientes. La UI cierra la confirmación al iniciar y ofrece «Cancelar eliminación». Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/projects.rs`, `src/bridge/delivery-contracts.ts`, `src/features/projects/projectModel.ts`, `src/features/projects/useProjectsController.ts`, `src/features/projects/ProjectsPanel.tsx`, `src/App.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: la lista de proyectos consulta cancelación por fila y al medir los bytes de snapshots e historial; una generación de solicitud frontend descarta respuestas obsoletas. La UI ofrece «Cancelar carga» y reintento. `get_recovery_candidate` sigue siendo una consulta separada de una fila sin token. `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check` pasan; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/projects.rs`, `src/bridge/delivery-contracts.ts`, `src/features/projects/projectModel.ts`, `src/features/projects/useProjectsController.ts`, `src/features/projects/ProjectsPanel.tsx`, `src/App.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: `list_projects` devuelve el catálogo y el candidato de recuperación en una sola respuesta y transacción de lectura bajo `projectCatalog`. La migración SQLite v15 crea el índice parcial; la lista y la consulta del candidato comparten la misma vista de lectura, y la consulta indexada comprueba cancelación antes y después de su tramo SQLite síncrono. Se elimina la invocación separada `get_recovery_candidate`; se actualizan el bridge, los mocks frontend/E2E, el smoke nativo y el inventario IPC. Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/projects.rs`, `src-tauri/src/lib.rs`, `src/bridge/project-contracts.ts`, `src/bridge/projects.ts`, `src/features/projects/useProjectsController.ts`, `src/features/projects/useProjectsController.test.tsx`, `src/App.test.tsx`, `e2e/`, `tools/probe-webview2-projects.mjs`, `tools/check-ipc-inventory.mjs`, `docs/reference/ipc-inventory.json`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: la vista previa `preview_delimited_header_review` comparte la generación `load` de la selección. Rust comprueba el token alrededor de la lectura acotada a 64 KiB, la detección de delimitador, el staging temporal, los dos parseos de muestra y la revalidación de tamaño; «Cancelar» también envía `cancel_operation("load")` antes de descartar la selección. Lectura y parseo siguen siendo síncronos, pero al retornar se descarta cualquier resultado cancelado. Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src/App.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: la detección del delimitador lee la muestra de 64 KiB en bloques de 8 KiB y revisa cancelación entre lecturas y entre los cuatro candidatos de delimitador. Tanto `preview_delimited_header_review` como la carga completa CSV/TSV/TXT propagan la generación `load`; la lectura Polars comienza solo tras terminar la detección. Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: el catálogo local de tareas reutilizables usa una generación cancelable, revisada entre filas SQLite y deserializaciones. El panel ofrece «Cancelar carga» y reintento, conserva el catálogo anterior al cancelar una actualización y descarta respuestas obsoletas. La lectura de una fila SQLite y su deserialización siguen siendo síncronas. Pasan `cargo fmt`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check` y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/reusable_tasks.rs`, `src-tauri/src/dataset.rs`, `src/bridge/delivery-contracts.ts`, `src/features/load/useReusableTasks.ts`, `src/features/load/ReusableTaskPanel.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV01: la consulta de versiones ahora lleva una generación asociada al proyecto activo; cambiar de proyecto o desmontar invalida respuestas y errores pendientes, y la UI oculta versiones cuyo ID no coincide con el proyecto activo. La retención limita el historial a cinco versiones. Pasan `npm run build` y `git diff --check`; no se ejecutaron pruebas de producto. | `src/features/projects/useProjectsController.ts`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: el catálogo de presets de entrega usa una generación cancelable y comprueba el token entre filas SQLite y deserializaciones. El panel permite cancelar y reintentar; mantiene el catálogo previo y descarta resultados obsoletos. El límite de 100 presets acota la lista, aunque la lectura y deserialización por fila siguen siendo síncronas. Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/delivery_presets.rs`, `src-tauri/src/dataset.rs`, `src/bridge/delivery-contracts.ts`, `src/features/delivery/DeliveryPhase.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV06: la tarjeta de progreso separa el anuncio vivo del panel visual. Un estado breve anuncia cambios de etapa y cancelación; la barra nativa conserva su nombre y valor, y el reloj/porcentaje dejan de disparar anuncios de toda la tarjeta. `npm run build`, el checker documental y `git diff --check` pasan; no se ejecutaron pruebas de producto. | `src/components/OperationProgressView.tsx`, `src/components/OperationProgressView.test.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV01/RV04: `list_project_versions` aplica `LIMIT 5` en SQLite y solo deserializa las versiones recientes, incluso si el catálogo contiene filas sobrantes. Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` (58 avisos dead-code existentes), `npm run build`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/projects.rs`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: recuperación de generaciones consulta referencias SQLite sin convertir errores en un conjunto vacío; si no puede leerlas, omite la limpieza. La inicialización y el recorrido/eliminación de huérfanos revisan cancelación. `list_project_versions` usa `projectVersions`, recorre hasta cinco entradas con cancelación y Proyectos ofrece cancelar/reintentar. `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run build`, `npm run ipc:check` y `git diff --check` pasan; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/project_recovery.rs`, `src-tauri/src/projects.rs`, `src/bridge/delivery-contracts.ts`, `src/features/projects/projectModel.ts`, `src/features/projects/useProjectsController.ts`, `src/features/projects/ProjectsPanel.tsx`, `src/App.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV01/RV04: inicio, cancelación y commit de la carga comparten `load_commit_lock`. La inspección publica selección/hojas solo bajo una generación vigente; tras crear el historial, la carga comprueba token e ID de selección y sustituye dataset, comparación y selección bajo el gate. Cancelar durante la creación síncrona del historial descarta el candidato y conserva la sesión anterior; si el commit entra primero, cancelar espera a esa publicación. Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` (58 avisos dead-code existentes), `npm run build`, `npm run ipc:check` y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/tests.rs`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: `test_database_connection` ya no bloquea el executor Tauri: ODBC se ejecuta en `spawn_blocking` bajo el token independiente `databaseConnection`, con comprobaciones antes/después de inicializar el entorno, abrir la conexión y ejecutar `SELECT 1`. El driver no interrumpe una llamada activa; si cancelar llega durante ella, el resultado se descarta al volver. El comando independiente no tiene un consumidor actual en la interfaz. Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` (58 avisos dead-code existentes), `npm run build`, `npm run ipc:check` y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/remote_databases.rs`, `src-tauri/src/dataset.rs`, `src/bridge/delivery-contracts.ts`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: la comprobación de actualizaciones de red usa el token `updateCheck` y el panel permite cancelarla. `tokio::select!` descarta la consulta activa; la cancelación y la publicación comparten un gate con generación para que solo una gane. Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` (58 avisos dead-code existentes), `npm run build`, `npm run ipc:check` (84 producción, 4 debug, 69 estructuras), el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/updater.rs`, `src-tauri/src/dataset.rs`, `src/bridge/delivery-contracts.ts`, `src/components/UpdatePanel.tsx`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: las exportaciones source-backed Parquet y JSON usan el monitor de cancelación de DuckDB durante `COPY`; la copia al temporal de publicación comprueba el token cada 64 KiB. `sync_all` permanece síncrono y el destino solo se publica al completar la copia. Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` (58 avisos dead-code existentes), `npm run build`, `npm run ipc:check` (84 producción, 4 debug, 69 estructuras), el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/duckdb_query.rs`, `src-tauri/src/dataset.rs`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: el hash SHA-256 del CSV temporal de Bundle comprueba cancelación al iniciar, entre lecturas de 64 KiB y después de rebobinar; se aplica a las rutas eager y source-backed. El Bundle incompleto no se publica y el conteo de bytes rechaza desbordamientos. Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` (58 avisos dead-code existentes), `npm run build`, `npm run ipc:check` (84 producción, 4 debug, 69 estructuras), el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: la protección eager de privacidad y las exportaciones CSV/JSON/Parquet observan cancelación. Los escritores Polars procesan bloques de 8.192 filas; CSV neutraliza fórmulas por bloque, JSON combina bloques en un solo arreglo y Parquet usa escritura batched. El Bundle eager utiliza el mismo escritor CSV cancelable; el archivo de destino solo se publica tras completar la exportación. El cierre del escritor y `sync_all` siguen síncronos. Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` (58 avisos dead-code existentes), `npm run build`, `npm run ipc:check` (84 producción, 4 debug, 69 estructuras), el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/dataset.rs`, `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV04: la reconciliación de generaciones huérfanas consulta cancelación al enumerar y validar entradas, y antes/después de cada `std::fs::remove_dir_all` seguro. La llamada recursiva sigue siendo síncrona: una cancelación durante ese syscall se reconoce al regresar. El borrado explícito continúa bajo el gate si el commit del catálogo ya ocurrió. Pasan `cargo fmt --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` (58 avisos dead-code existentes), el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src-tauri/src/project_recovery.rs`, `CONTEXTO.md`, `ROADMAP.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV06: los diálogos propios usan `<dialog>.showModal()` y la capa modal nativa del navegador, que deja inerte el contenido de fondo; se conservan el nombre/descripción ARIA, el trap de teclado, Escape y la restauración del foco. La aceptación con lector de pantalla real sigue pendiente. Pasan `npm run build`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src/components/ModalDialog.tsx`, `src/styles.css`, `CONTEXTO.md`, `ROADMAP.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV06: el error de vista previa CSV/TSV se anuncia dentro del diálogo de revisión; se retiró la copia global redundante que quedaba fuera de contexto. El estado del criterio permanece parcial hasta la aceptación nativa con lector de pantalla. Pasan `npm run build`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src/features/load/LoadPhase.tsx`, `CONTEXTO.md`, `ROADMAP.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV06: las etiquetas visibles de los campos ODBC de esquema y tabla coinciden ahora con sus nombres accesibles; la opcionalidad del esquema se conserva en la ayuda visible y el mensaje de validación sigue asociado con el control por `aria-describedby`. La aceptación con lector de pantalla continúa pendiente. Pasan `npm run build`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src/features/delivery/DeliveryPhase.tsx`, `CONTEXTO.md`, `ROADMAP.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV06: la alerta de validación del contrato de calidad queda asociada mediante `aria-describedby` al fieldset de la regla cuyo número aparece en el mensaje; los errores generales del contrato conservan el anuncio global. No se mueve el foco durante la edición. La aceptación nativa con lector de pantalla continúa pendiente. Pasan `npm run build`, el checker documental y `git diff --check`; no se ejecutaron pruebas de producto. | `src/features/delivery/DeliveryPhase.tsx`, `CONTEXTO.md`, `ROADMAP.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV10: comprobación local de precondiciones para el round-trip. ODBC Driver 17/18 y `sqlcmd` están instalados; `MSSQLSERVER` está detenido y Windows rechazó abrir el servicio desde esta sesión. La instancia `(localdb)\MSSQLLocalDB` tampoco respondió. No se creó ni modificó una base y el round-trip sigue pendiente de una instancia accesible. | `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV02/RV06: el mock Playwright de importación ahora responde `inspect_workbook_sheets`, reflejando la inspección Excel separada. El caso Excel aislado pasa y la suite Playwright completa pasa 21/21, con cobertura de importación CSV/XLSX/Parquet, teclado, asociación del error ODBC y zoom 200 %. La aceptación con datasets de trabajo reales y lector de pantalla nativo sigue pendiente. | `e2e/import-formats.spec.ts`, `CONTEXTO.md`, `ROADMAP.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV06: una auditoría de etiquetas detectó nombres accesibles incompletos en límites, referencias de reglas, orden del esquema y el selector de preset. Los nombres ARIA ahora incluyen el texto visible completo y distinguen cada control por regla; se actualizaron las consultas existentes. `DeliveryPhase.test.tsx` pasa 32/32 y Playwright pasa 21/21, incluidos teclado y zoom 200 %. La aceptación con lector de pantalla nativo sigue pendiente. | `src/features/delivery/DeliveryPhase.tsx`, `src/features/delivery/DeliveryPhase.test.tsx`, `CONTEXTO.md`, `ROADMAP.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV05: guardar una tarea ahora comprueba automáticamente que su esquema coincide con el dataset activo y habilita aplicarla sin volver a seleccionarla. La prueba del panel pasa 7/7; Playwright completa el recorrido de importación, guardado, revisión del esquema y aplicación, y la suite pasa 22/22 con build incluido. La aceptación con archivos de trabajo y persistencia entre sesiones sigue pendiente. | `src/features/load/ReusableTaskPanel.tsx`, `src/features/load/ReusableTaskPanel.test.tsx`, `e2e/import-formats.spec.ts`, `CONTEXTO.md`, `ROADMAP.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | El proyecto sube de 0.167.0 a 0.168.0 con versiones npm, Cargo, Tauri y documentación sincronizadas. Se mantiene en la serie 0.x mientras RV07 (beta con datos reales), RV09 (lector de pantalla nativo), RV10 (SQL Server) y RV11 (distribución binaria) sigan abiertos; el cambio de versión no declara esas aceptaciones ni publica artefactos. | `package.json`, `package-lock.json`, `src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`, `src-tauri/tauri.conf.json`, `CHANGELOG.md`, `CONTEXTO.md`, `AUDITORIA.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | A petición del usuario, la versión del proyecto pasa de 0.168.0 a 1.25.0 para representar mejor los cambios acumulados. Manifiestos, fixtures y documentación quedan sincronizados; la numeración 1.x no implica que estén cerrados los gates RV07 (beta con datos reales), RV09 (lector de pantalla nativo), RV10 (SQL Server) y RV11 (distribución binaria), ni que se publiquen artefactos. | `package.json`, `package-lock.json`, `src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`, `src-tauri/tauri.conf.json`, `CHANGELOG.md`, `CONTEXTO.md`, `AUDITORIA.md`, `docs/reference/roadmap-current.md` |
...
| 2026-09-21 | RV12/RV01: el benchmark WebView2 de 100 MiB pasa con 819.137 filas y confirma carga, paginación, transformación, exportación, memoria dentro del presupuesto del benchmark y cleanup. La evidencia está en `.local/validation/performance-webview2/20260921T230202Z/summary.json`; el presupuesto de memoria del ejecutable release sigue siendo un gate separado. | `tools/benchmark-webview2-dataset.ps1`, `tools/probe-webview2-cdp.ps1`, `CONTEXTO.md`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `.local/validation/performance-webview2/20260921T230202Z/summary.json` |
| 2026-09-21 | RV01/RV05/RV06: `npm run smoke:cdp` pasa Playwright, landmarks, foco y ProjectsPanel, además de 18 operaciones IPC nativas de receta, exportación, reapertura, restauración de fase y cleanup. La evidencia está en `.local/validation/webview2-cdp/20260921T230251Z/summary.json`; la memoria del runtime debug queda como diagnóstico y la aceptación con tareas reales/lector nativo sigue pendiente. | `tools/probe-webview2-cdp.ps1`, `tools/probe-webview2-projects.mjs`, `CONTEXTO.md`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `.local/validation/webview2-cdp/20260921T230251Z/summary.json` |
| 2026-09-21 | RV01/RV02/RV05/RV06: se sincronizan las regresiones de `App` con el contrato vigente: la inspección de hojas Excel se mockea como llamada separada, la fase Preparar espera su carga perezosa y Entregar espera el análisis antes de navegar; también se actualiza el nombre accesible del límite de inválidos. La suite frontend pasa `426/426` con `npx vitest run --maxWorkers=1`, Playwright pasa `22/22` y su build incluido; los gates beta, legal e IPC pasan. La versión vigente del proyecto sigue siendo `1.25.0`; RV07/RV09/RV10/RV11 permanecen abiertos por evidencia externa. | `src/App.test.tsx`, `e2e/import-formats.spec.ts`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV11: `npm run release:dry-run` sobre `1.25.0` confirma árbol limpio, toolchains, documentación, inventario IPC, gobernanza y gate técnico legal; se detiene en el sign-off jurídico obligatorio antes de empaquetar. No crea ni publica artefactos. La evidencia queda en `.local/validation/20260921T232220Z-a8fb7b4-package.json`; RV11 sigue parcial hasta contar con aprobación jurídica, VM limpia, artefactos y verificación del canal. | `tools/release.ps1`, `tools/check.ps1`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `.local/validation/20260921T232220Z-a8fb7b4-package.json` |
...
| 2026-09-21 | RV16: se extrae la comparación de revisiones del historial y deltas de calidad a `dataset/snapshot_comparison.rs`; las tres pruebas dedicadas pasan y la suite Rust completa pasa 494/499 (cinco ignoradas por servicios/drivers externos). `cargo check --tests` también pasa. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/snapshot_comparison.rs`, `src-tauri/src/dataset/snapshot_comparison_tests.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV16: el cuarto corte extrae los tipos, el parser y la planificación de consultas locales de `dataset.rs` a `dataset/local_query.rs`, manteniendo los contratos. `cargo check --tests` pasa; queda pendiente ejecutar la suite completa tras este corte. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/local_query.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV16: el quinto corte extrae el cálculo de estadísticas numéricas del perfil a `dataset/numeric_profile.rs`, incluidos valores, histogramas y cuantiles con almacenamiento temporal acotado. Se mantienen los contratos; `cargo check --tests` pasa. La suite completa no se ejecutó para este corte. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/numeric_profile.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV16: el sexto corte extrae el resumen categórico eager y source-backed a `dataset/categorical_profile.rs`; ambas rutas comparten el filtrado, orden, límite y cálculo de «Resto». `cargo check --tests` pasa; no se ejecutó la suite completa tras esta extracción. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/categorical_profile.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV10: el intento de iniciar `MSSQLSERVER` desde esta sesión falla con `Cannot open 'MSSQLSERVER' service on computer '.'`; el puerto 1433 no responde y no hay comandos Docker/Podman disponibles. El servicio queda detenido; no se creó ni modificó una base. El round-trip sigue pendiente de una instancia accesible. | `CONTEXTO.md`, `docs/reference/roadmap-current.md` |
| 2026-09-21 | RV16: el séptimo corte extrae el perfil temporal eager/source-backed y los helpers de periodos a `dataset/temporal_profile.rs`; la agregación temporal reutiliza sus tipos y helpers, y ambos perfiles comparten el postprocesamiento. `cargo check --tests` pasa y, con el manifiesto Common Controls v6, `cargo test --lib` pasa 494/499 (dos benchmarks opt-in y tres integraciones ODBC externas ignorados). | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/temporal_profile.rs`, `src-tauri/src/dataset/tests.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV16: el octavo corte extrae las correlaciones numéricas eager y source-backed a `dataset/numeric_profile.rs`, junto con el muestreo y Pearson; conserva API, límites de muestra y comportamiento. `cargo check --tests`, formato y la suite Rust completa pasan: 494 aprobadas, 0 fallidas y 5 ignoradas (dos benchmarks opt-in y tres integraciones ODBC externas). La versión sigue en `1.25.0`; RV16 continúa abierta para modularización gradual. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/numeric_profile.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV16: el noveno corte extrae los modelos públicos de reglas de calidad y el helper del resultado a `dataset/quality_contracts.rs`; `dataset.rs` reexporta los mismos tipos con sus rutas actuales. No cambia la forma JSON ni el comportamiento. `cargo check --tests` pasa; la suite Rust completa registra 494 aprobadas, 0 fallidas y 5 ignoradas (dos benchmarks opt-in y tres integraciones ODBC externas). La versión continúa en `1.25.0`; RV16 sigue abierta. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/quality_contracts.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV16: el décimo corte extrae migración, validación, importación y persistencia de documentos de reglas de calidad a `dataset/quality_documents.rs`. Se conservan APIs de los comandos y migraciones compatibles; la carga y el guardado siguen acotados y atómicos. `cargo check --tests` pasa y `cargo test --lib` registra 494 aprobadas, 0 fallidas y 5 ignoradas. La versión continúa en `1.25.0`; RV16 permanece en curso. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/quality_documents.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV16: el undécimo corte extrae la validación y evaluación de calidad eager/source-backed a `dataset/quality_evaluation.rs`, junto con reglas, agregaciones, cancelación y enforcement previo a la exportación. Los helpers compartidos con tendencias temporales conservan su uso interno; rutas públicas, API y comportamiento se preservan. `cargo check --tests` pasa y `cargo test --lib` registra 494 aprobadas, 0 fallidas y 5 ignoradas. La versión continúa en `1.25.0`; RV16 permanece en curso. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/quality_evaluation.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md`
| 2026-09-21 | RV16: el duodécimo corte extrae la validación y ejecución lazy de recetas a `dataset/recipe_engine.rs`, incluido el despacho al fallback eager y la comprobación de renombrados compartida con la ruta source-backed. `cargo fmt --check` y `cargo check --tests` pasan; la suite Rust completa registra 494 aprobadas, 0 fallidas y 5 ignoradas. La versión continúa en `1.25.0`; RV16 permanece en curso. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/recipe_engine.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md`
| 2026-09-21 | RV16: el decimotercer corte extrae la planificación y ejecución source-backed de recetas a `dataset/recipe_source_projection.rs`. Se mantienen el despacho, la cancelación, la salida Parquet y los helpers DuckDB compartidos con otras operaciones; `cargo fmt --check`, `cargo check --tests` y la suite Rust completa pasan (494 aprobadas, 0 fallidas y 5 ignoradas). La versión permanece en `1.25.0`. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/recipe_source_projection.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV05: `npm run smoke:restart` pasa en dos sesiones reales de Columnia y confirma que una tarea reutilizable sintética persiste en el catálogo SQLite, se lista y reabre con sus campos intactos tras el reinicio, y se elimina al final. El catálogo queda 0→1→0; también se conserva `activePhase=prepare`. Evidencia: `.local/validation/webview2-restart/20260922T015059Z/summary.json` y las dos fases CDP asociadas. El pico de memoria privada debug fue 302.895.104 bytes frente al presupuesto diagnóstico de 268.435.456 bytes, que no es requisito de este smoke. RV05 sigue parcial hasta validar tareas y archivos de trabajo; la versión permanece en `1.25.0`. | `tools/probe-webview2-projects.mjs`, `tools/probe-webview2-cdp.ps1`, `tools/probe-webview2-restart.ps1`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md`, `.local/validation/webview2-restart/20260922T015059Z/summary.json` |
| 2026-09-21 | RV05: se amplía `npm run smoke:native-selectors` con un recorrido Playwright en WebView2 y el selector Win32. La tarea sintética se guarda y abre desde el panel; un CSV con la columna adicional `extra` muestra las diferencias y requiere confirmación antes de importar; luego el CSV compatible carga la receta guardada como borrador y lleva a Preparar. La limpieza elimina la tarea (catálogo vuelve a su estado previo). Evidencia UTC: `.local/validation/webview2-cdp/20260922T022017Z/native-selectors.stdout.log` y `.local/validation/webview2-cdp/20260922T022017Z/summary.json`. El pico de memoria privada del runtime debug fue 441.495.552 bytes frente al presupuesto diagnóstico no aplicado de 268.435.456 bytes. RV05 sigue parcial hasta la aceptación con flujos y archivos de trabajo reales; versión `1.25.0`. | `tools/probe-webview2-native-selectors.mjs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md`, `.local/validation/webview2-cdp/20260922T022017Z/summary.json` |
| 2026-09-21 | RV16: se extrae la protección de fórmulas CSV/Bundle a `dataset/csv_formula_safety.rs`; se preservan prefijos neutralizados, valores nulos, tipos no textuales y comprobaciones cooperativas de cancelación cada 4.096 filas. JSON, Excel, SQL, SQLite y Parquet mantienen sus valores previos. `cargo fmt --all -- --check`, `cargo check --tests` y `cargo test --lib` con el harness Windows Common Controls v6 pasan: 494 aprobadas, 0 fallidas y 5 ignoradas (dos benchmarks opt-in y tres integraciones ODBC externas). La versión permanece en `1.25.0`; RV16 sigue en curso. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/csv_formula_safety.rs`, `src-tauri/src/dataset/tests.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
...
| 2026-09-22 | RV02/RV04/RV16: selección manual, drag/drop, enumeración de hojas y revisión previa de encabezados pasan a `dataset/import_source_inspection.rs`. Los cuatro comandos Tauri conservan nombre, parámetros y ruta; la cancelación permanece ligada a la generación de carga, y `samples` conserva su helper mediante un puente privado. RV16 suma diecinueve extracciones. `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run ipc:check` (85/4/70), `node tools/check-documentation.mjs` y `git diff --check` pasan. No se ejecutaron pruebas de producto; versión `1.26.0`. RV02 sigue pendiente de datasets reales y RV16 continúa abierta. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/import_source_inspection.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-22 | RV06: el historial de Preparar conserva visible el último resultado y los controles Deshacer/Rehacer, pero ya no despliega automáticamente la lista de cambios; su resumen nativo permite abrirla con teclado y libera espacio en el panel. La regresión de diseño existente se actualizó, pero no se ejecutó. `npm run build` pasa; no se ejecutaron pruebas de producto. Versión `1.26.0`; RV06 sigue parcial hasta aceptación nativa con lector de pantalla. | `src/features/prepare/HistoryBar.tsx`, `src/features/prepare/HistoryBar.design.test.tsx`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-22 | RV02/RV04/RV16: la carga final y el descarte de selecciones pasan a `dataset/import_loading.rs`. Los comandos Tauri conservan nombres, parámetros y rutas; siguen intactos la validación del archivo y perfil, las decisiones de hoja/encabezado/convenciones, la cancelación por generación, la construcción del candidato y el gate atómico que publica dataset, limpia comparación y consume la selección. RV16 suma veinte extracciones. `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib`, `npm run ipc:check` (85/4/70), `node tools/check-documentation.mjs` y `git diff --check` pasan. No se ejecutaron pruebas de producto; versión `1.26.0`. RV02 continúa pendiente de aceptación con datasets reales; RV04 conserva límites síncronos nativos y RV16 sigue abierta. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/import_loading.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-22 | RV16: `get_dataset_page` y su orquestación pasan a `dataset/page_reader.rs`, junto con sus lectores eager, Parquet y source-backed. Se conserva el wrapper Tauri, la generación `datasetPage`, la lectura por snapshot y fallbacks, la comprobación de fuente activa, los límites y errores. `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` (58 advertencias de código inactivo, sin errores), `npm run ipc:check` (85/4/70), `node tools/check-documentation.mjs` y `git diff --check` pasan. No se ejecutaron pruebas de producto; versión `1.26.0`; RV16 conserva veinte módulos y veintiuna responsabilidades extraídas, y continúa abierta. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/page_reader.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-22 | RV16: `get_dataset_profile` delega su orquestación en `dataset/profile_reader.rs`. El wrapper Tauri mantiene nombre, firma y parámetros; se preservan cache validado, lectura del snapshot, fallbacks source-backed/eager y cancelación por generación. `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` (58 advertencias de código inactivo, sin errores) y `npm run ipc:check` (85/4/70) pasan. No se ejecutaron pruebas de producto; versión `1.26.0`; RV16 queda en veintiún módulos y veintidós responsabilidades extraídas, y continúa abierta. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/profile_reader.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |

...
> Estado verificado el 2026-09-23 (reauditoría Tier 10). Este documento describe el sistema implementado, no una garantía absoluta de seguridad. Debe actualizarse cuando cambien datos, IPC, permisos, red, parsers, persistencia o distribución.
...
| SQL local (Revisar) | Consulta escrita por la persona o por una UI comprometida que intente leer archivos ajenos, usar la red o instalar extensiones mediante DuckDB | Subconjunto SQL validado (`dataset/local_query.rs`); Polars ejecuta el plan validado; DuckDB recibe solo SQL reconstruido desde el plan, con identificadores entre comillas y literales re-escapados (T10-03); la conexión solo permite los archivos del dataset y su temporal, sin acceso externo ni extensiones y con la configuración bloqueada (T10-04) | Las consultas de JOIN y consolidación de Revisar generan SQL interno desde nombres de columna citados, sin la restricción de rutas de T10-04. |
| Entrega ODBC (PostgreSQL, MySQL, SQL Server) | Salida de datos personales a un servidor remoto; credenciales en la cadena de conexión; canal sin cifrar; servidor lento o bloqueado; duplicados al reintentar `append`; UI comprometida que exfiltra el dataset | Cadena de conexión nunca persistida; errores con secretos redactados, también entre llaves (T10-33); identificadores ASCII validados y parámetros enlazados; preflight que bloquea sin cifrado exigido (T10-06); timeouts de conexión (15 s) y de sentencia (300 s) (T10-13); `Replace` desactivado en MySQL; `odbc_api` limitado a `remote_databases.rs` por el gate de red (T10-07) | Cada conexión remota exige confirmar el destino en un diálogo nativo que la WebView no puede falsificar; la confirmación se recuerda por huella SHA-256 durante la sesión (T10-05). Un `append` repetido con el mismo contenido en la misma tabla, incluido un reintento tras un commit dudoso, exige confirmar en un diálogo nativo; el registro local `remote-deliveries.json` solo guarda claves SHA-256 sin secretos (T10-14). La inserción va por lotes de hasta 2.000 parámetros. El registro es por equipo: otra instalación no lo ve. Un proceso local que sondeaba UI Automation durante varios segundos sin tocar el diálogo de confirmación llegó a resolverlo como aceptado (observado en el smoke nativo del 2026-09-25). No es alcanzable desde la WebView, y ese proceso podría pulsar el botón de todos modos. Los textos de aviso requieren revisión legal (Ley 172-13). |
| Catálogos locales (tareas reutilizables y presets de entrega) | Documentos manipulados, credenciales o permisos de sobrescritura persistidos por error | SQLite propio bajo app-data; esquemas estrictos que rechazan credenciales y autorización de sobrescritura; tamaños acotados; catálogos cancelables | Los smokes nativos escriben en el catálogo real; una corrida abortada puede dejar entradas sintéticas (T10-17). |
...
- SQL de usuario confinado: DuckDB solo recibe el plan reconstruido y no accede a archivos ajenos, red ni extensiones.
- Entregas remotas con cifrado exigido o expresamente desactivado, timeouts explícitos y secretos redactados.
...
- [ ] ¿Abre una salida de red o de datos (ODBC, updater, extensiones DuckDB)? Si sí, ¿está en `declaredEgress` de `tools/check-network-policy.mjs` y exige una acción explícita?

...
| CLI local | Rutas manipuladas, hoja ambigua, receta/contrato inválido, sobrescritura parcial o fuga de rutas/datos en resultados automatizados | Parser estricto; canonicalización y límite de 500 MiB compartidos; hoja exacta y encabezado obligatorio para libros; documentos de reglas v1 sin campos desconocidos y con presupuestos semánticos; exportación atómica; JSON de conteos sin rutas ni muestras; smoke real de CSV/Parquet/XLSX y calidad pass/fail | La CLI hereda la autoridad de la cuenta local y no es un sandbox ni autenticación. La persona puede apuntar a archivos sensibles. Los documentos JSON se limitan semánticamente después de deserializar y no existe todavía ejecución por lotes/proyectos. |

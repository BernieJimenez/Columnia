...
- 2026-08-29 la importación de sesiones prioriza un `snapshot_path` local y compatible cuando existe, porque conserva el estado materializado exacto y no requiere inventar parámetros para `applied_ops`; sin snapshot, la fuente se valida, la receta estructural se reaplica y `impute_categorical` se reproduce por su semántica fija. Historial, cachés y resultados de análisis continúan fuera del contrato.

...
del commit `85934c9`. La versión identifica el alcance del producto y no implica
que estén cerrados los gates de beta con datos reales, accesibilidad nativa, SQL
Server o distribución binaria. RV16 ya separa nueve responsabilidades: validación
...
documentos (`dataset/quality_documents.rs`), resúmenes categóricos
...
la ruta anterior y mantienen el mismo JSON; el parser de fechas compartido queda
accesible dentro del motor. Los límites de muestreo, la API y los contratos se
conservan. `cargo check --tests`, `cargo fmt --check`, el verificador documental
y la suite Rust completa pasan: 494 aprobadas, 0 fallidas y 5 ignoradas (dos
benchmarks opt-in y tres integraciones ODBC externas). RV16 sigue abierta para
...
| --- | --- |
| Última actualización | 2026-09-22; este corte parte de `85934c9`; RV16 extrae migración y persistencia de documentos de reglas a `quality_documents.rs`. `cargo check --tests`, formato, documentación y suite Rust completa pasan (494 aprobadas, 5 ignoradas); cola vigente en `docs/reference/roadmap-current.md` |
...
| Última revisión de este documento | 2026-09-22, posterior al commit base `85934c9`; registra el décimo corte de RV16 y la suite Rust completa (494 pasaron, 5 ignoradas). La versión vigente continúa en `1.25.0`. La cola y los límites abiertos están detallados en `docs/reference/roadmap-current.md`; las secciones fechadas más abajo son registro histórico. |

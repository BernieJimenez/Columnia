...
| Pruebas observadas | 286 frontend y 300 Rust aprobadas en la suite local actual; E2E y Package históricos pasan en la estación auditada; smoke nativo Win32 y smoke NSIS instalado pasan con cleanup y presupuesto de memoria |
...
No presentes como implementada una tecnología solo porque aparece en el roadmap. DuckDB ya forma parte de las dependencias de Cargo y ofrece la primera ruta opcional de consulta SQL local: reutiliza el snapshot Parquet administrado de la revisión actual cuando existe y conserva un fallback temporal para estados degradados; la ejecución incremental general y el procesamiento fuera de la RAM del dataset siguen pendientes.

...
- Las suites locales actuales pasan: 281 tests frontend y 262 tests Rust; los
...
| 2026-08-29 | M1/P1 incorpora `mask_pii` al fallback de sesiones DataPrep: sin snapshot compatible se aplica la máscara predeterminada y conservadora `[REDACTED]` a valores no nulos de columnas personales detectadas, preservando nulos y `_cambios`; snapshots compatibles siguen teniendo prioridad y los modos hash/clave explícita requieren revisión. | `src-tauri/src/dataset.rs`, `src-tauri/src/projects.rs`, `docs/reference/migration-inventory.md` |

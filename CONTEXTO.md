...
- Fase P1 activa: matriz de paridad con `dataprepv1.1`, exportación JSON/SQL/Excel/SQLite atómica, privacidad de salida, comparación por clave/conflictos, joins `Inner`/`Left`/`Full`, consulta local restringida con filtros/agregaciones y visualizaciones accesibles disponibles en UI/Rust; siguen pendientes análisis exploratorio amplio, reglas de calidad versionadas, conectores remotos, bundles auditables, escala fuera de memoria y resolución interactiva de conflictos.

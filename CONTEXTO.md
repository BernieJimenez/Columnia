...
| --- | --- |
| Última actualización | 2026-09-22; este corte parte de `15e4ff6`; RV04 conserva la selección pendiente y permite reintentar cancelación/liberación sin perder `selectionId`; RV02 revisa esquema candidato y discrepancias antes de activar, con datasets reales pendientes; RV05 conserva evidencia de reinicio sintético; RV16 mantiene diecisiete módulos, incluidos los contratos de historial recién extraídos. `npm run build` pasó en el corte RV04; `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check` y `cargo check --manifest-path src-tauri/Cargo.toml --lib` pasan en esta extracción. No se ejecutaron pruebas de producto. Cola vigente en `docs/reference/roadmap-current.md` |
...
| Última revisión de este documento | 2026-09-22, posterior al commit base `15e4ff6`; documenta el reintento de cancelación/liberación de selecciones pendientes de RV04 y la extracción de contratos de historial de RV16. `npm run build` pasa en el corte RV04; `cargo fmt --check` y `cargo check --manifest-path src-tauri/Cargo.toml --lib` pasan en este corte. No se ejecutaron pruebas de producto. La versión vigente es `1.26.0`; RV04 sigue parcial y los gates abiertos están en `docs/reference/roadmap-current.md`. |

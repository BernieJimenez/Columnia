...
- consulta SQL local de solo lectura con filtros, agregaciones por bloques, `GROUP BY` compuesto de hasta ocho columnas y `JOIN` de hasta ocho pares de claves, con paginación, orden estable, claves nulas y límites de cardinalidad;
- comparación por claves con índice temporal particionado, conflictos paginados y consolidación de nuevas claves sin retener el índice completo en memoria;

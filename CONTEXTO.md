...
| --- | --- |
| Última actualización | 2026-09-23; RV04 conserva selecciones fallidas para reintento mediante `selectionId` y cancela por lotes la lectura Parquet en consultas, conflictos, apertura de proyectos y vistas source-backed de recetas; no entrega resultados parciales. RV04 sigue parcial por aceptación nativa/datos reales y APIs síncronas.; RV02 revisa esquema candidato y discrepancias antes de activar, con aceptación en datasets reales pendiente; RV05 conserva evidencia de reinicio sintético; RV06 pliega por defecto la lista del historial de Preparar y deja visible su último resultado y Deshacer/Rehacer; RV16 mantiene treinta y ocho módulos con responsabilidades extraídas; `dataset/page_reader.rs` concentra el handler de paginación y sus fallbacks; `dataset/profile_reader.rs` concentra perfil y agregación temporal; `dataset/comparison_reader.rs` reúne comparación inicial y página de conflictos; `dataset/import_loading.rs` concentra carga final y descarte con publicación atómica; `dataset/import_source_inspection.rs` agrupa la selección y revisión de fuentes. `dataset/history.rs` reúne el historial, Deshacer/Rehacer con restauración cancelable y `dataset/import_schema_preview.rs` contiene el preflight de esquema; `dataset/source_loading.rs` concentra carga source-backed y materialización cancelable; `dataset/spreadsheet_io.rs` concentra inspección de libros, conversión tipada y snapshots por bloques; `dataset/export_io.rs` concentra escritores, privacidad y publicación atómica. `dataset/recipe_eager.rs` concentra la validación y aplicación eager de recetas. `dataset/comparison_engine.rs` concentra comparación, conflictos y JOIN. `dataset/query_execution.rs` concentra planes source-backed, lectura por bloques Parquet, agregaciones y JOIN local con cancelación. `dataset/automation.rs` concentra carga, inspección, transformación, validación y exportación del CLI, incluidos los caminos source-backed. `dataset/project_history.rs` concentra captura, restauración y resúmenes de snapshots de proyectos. `dataset/profile_engine.rs` concentra inferencia de tipos, estadísticas de texto, perfilado eager/source-backed y agregaciones temporales. `npm run build` pasó en RV04; `npm test` pasó 457/457 y la suite Rust pasó 494/494 ejecutables con 5 ignoradas; `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y `git diff --check` pasan en RV16. La aceptación con datos de trabajo reales sigue pendiente. Cola vigente en `docs/reference/roadmap-current.md` |
| Producto | Estación de escritorio local para revisar, limpiar, transformar y entregar datasets confiables |
| Extracción RV16 vigente | `dataset/recipe_eager.rs` concentra la validación y aplicación eager de recetas; `dataset/comparison_engine.rs` concentra índices, comparación, conflictos y JOIN; `dataset/query_execution.rs` concentra planes source-backed, bloques Parquet, agregaciones y JOIN local con cancelación; `dataset/automation.rs` concentra los helpers de inspección, transformación, validación y exportación del CLI; `dataset/project_history.rs` concentra captura, restauración y resúmenes de snapshots de proyectos; `dataset/comparison_io.rs` concentra la carga y persistencia temporal de fuentes comparadas; `dataset/profile_engine.rs` concentra inferencia de tipos, estadísticas de texto, perfilado eager/source-backed y agregaciones temporales; los módulos extraídos conservan los contratos Tauri y la cancelación existente. |
...
| --- | --- | --- |
| 2026-09-25 | Revisión de diseño completa (51 capturas: 17 estados en oscuro, claro y 900 × 640, el tamaño mínimo de ventana). Aplicado: el diálogo de importación empieza por la decisión (encabezados y muestra) y deja memoria y disco en «Detalles técnicos»; se quitan la fila «Paso N de 4» y los rótulos sobre cada título (la barra lateral marca el paso); Entregar exporta en una columna; un solo nombre por acción («Ver cambios propuestos», «Continuar a X»); contenido anidado sangrado; «Seleccionar otro dataset» secundario; Preferencias con el tema primero y sin panel de actualizaciones si no hay actualizador; editor de reglas sin lista duplicada; diálogos sin la sombra ámbar; menos jerga. No aplicado a propósito: el bloque «Espacio actual» es el shell informativo de espacios de trabajo, una decisión de producto con pruebas propias (incluida su visibilidad en la vista compacta), y la franja superior solo aparece al ancho mínimo exacto de 900 px. | `src/features/load/LoadPhase.tsx`, `src/App.tsx`, `src/workflow-styles.css` |
| 2026-09-24 | T10-11: los estilos oscuros dependían de dos mecanismos a la vez (bloques `@media (prefers-color-scheme: dark)` sin condición de tema y reglas `[data-theme="dark"]`), así que el aspecto variaba con el SO. Ahora la app fija `data-resolved-theme` (Sistema → light/dark; los demás temas conservan su nombre) en `index.html` y en `applyThemePreference`, sigue los cambios del SO con Sistema y todas las reglas oscuras cuelgan de ese atributo. Las reglas que venían del `@media` usan `:where(...)`, así que conservan su especificidad y la cascada no cambia. Verificado con instantáneas de colores de cada elemento en las cuatro fases: Claro, Oscuro y Sistema (con SO claro y oscuro) coinciden al 100 % con sus referencias, y Papel, Océano y Pizarra quedan igual con SO claro y dejan de cambiar con SO oscuro (antes 14 diferencias cada uno). Decisión: el aspecto de referencia de «Oscuro» es el de Oscuro con SO oscuro. | `src/styles.css`, `src/workflow-styles.css`, `src/features/settings/themeModel.ts`, `e2e/theme-contrast.spec.ts` |
| 2026-09-24 | Cargar completa la simplificación de las cuatro fases: sin párrafo de cabecera, formatos y «sin límite fijo de tamaño» en una línea, y sin pie cuando no hay dataset (antes mostraba «Siguiente paso» sin botón). Medido con el dataset de los E2E: Cargar 119 → 79 palabras y cabe sin desplazamiento (900 px). Totales frente al inicio: Cargar 119 → 79, Revisar 247 → 88, Preparar 193 → 98, Entregar 205 → 139. | `src/features/load/LoadPhase.tsx`, `src/App.tsx` |
| 2026-09-24 | Recorrido nativo de Preparar con un CSV sintético de 111 MiB (1.801.806 filas) en WebView2 con el binario debug. Destapó tres defectos que las suites no veían: (1) el análisis de calidad retenía el candado `current` durante minutos y la carga esperaba en `get_history_state`, así que la app no pasaba de Cargar (T10-16 confirmado); ahora analiza sin el candado y solo guarda en caché si el dataset no cambió; (2) el frontend comparaba tipos con `String`/`Int64` mientras el motor envía `str`/`i64`, lo que ocultaba recortar y rellenar en la app real; (3) el análisis en segundo plano deshabilitaba la navegación sin explicación. Tras corregirlos: «Aplicar 4 cambios» deja 1.800.000 filas, 0 vacíos y 0 duplicados en 204 s (debug), y `get_app_info` responde con mediana 2,3 ms y máximo 21,9 ms en 3.266 muestras durante la operación. Los mocks de pruebas siguen usando los nombres largos; `src/dataTypes.ts` acepta ambos. | `.local/validation/webview2-cdp/20260924T182652Z`, `tools/probe-webview2-native-selectors.mjs` (`--prepare-flow`), `src/dataTypes.ts` |
| 2026-09-24 | T10-25: linter frontend con oxlint 1.85.0 (MIT, una sola dependencia de desarrollo y sin la API del compilador de TypeScript, que `typescript-eslint` necesita y TypeScript 7 cambia). Reglas `rules-of-hooks` y `correctness` como error y `exhaustive-deps` como aviso (9 avisos abiertos). Las reglas de preparación para React Compiler (`set-state-in-effect`, `refs`; 41 hallazgos) quedan desactivadas como trabajo futuro: exigen reescribir efectos y no forman parte del criterio. `tsconfig.app.json` compila `src` sin pruebas ni tipos de Node/Vitest. | `.oxlintrc.json`, `tsconfig.app.json`, `package.json`, `tools/check.ps1` |
| 2026-09-24 | T10-18: primera línea base de cobertura Rust (`cargo llvm-cov --lib`, 511 pruebas): 70,94 % de líneas en 15 módulos; los más bajos son `updater` (25,45 %), `resource` (34,01 %) y `remote_databases` (54,1 %), y `dataset` queda en 69,45 %. Se registra sin umbral hasta tener historial. En el frontend, `deliveryModel.ts` (78,86 % de sentencias) queda por debajo del 80 % del resto de archivos críticos; su umbral es un piso de no retroceso que debe subir con más pruebas. | `tools/check.ps1` (paso «Rust coverage baseline»), `tools/summarize-rust-coverage.mjs`, `tools/check-coverage.mjs` |
| 2026-09-24 | Entregar pierde texto repetido sin tocar sus salvaguardas: sin párrafo de cabecera ni subtítulos explicativos, sin las etiquetas «Disponible/Ruta seleccionada» (el botón de opción ya indica la elección) y con la advertencia de SHA-256 solo cuando se elige el hash. Se conservan las confirmaciones de exportar sin validar y de datos personales. Medido con el dataset de los E2E: 205 → 139 palabras. | `src/features/delivery/DeliveryPhase.tsx` |
| 2026-09-24 | Revisar sigue el mismo criterio que Preparar: un resumen (filas, columnas, tamaño; nulos, duplicados y tipos incompatibles) y una sola acción, «Ver cambios propuestos», que lleva a la propuesta de Preparar. La barra de completitud, las prioridades explicadas, «Filas analizadas», el análisis detallado, el perfil por columna, la configuración y el SQL local pasan a un único desplegable «Más análisis y herramientas»; «Comparar con otro dataset» queda aparte por ser otro flujo. Medido con el dataset de los E2E: 247 → 88 palabras, 12 → 9 controles, 11 → 7 bloques. | `src/features/review/ReviewPhase.tsx` (`ReviewMoreTools`) |
| 2026-09-24 | Preparar pasa a una propuesta única: Columnia analiza y propone los cambios (marcadores de «sin dato», espacios, duplicados y rellenar vacíos), muestra ejemplos antes → después y espera siempre el clic del usuario; nunca aplica cambios por sí sola, tampoco al cargar (decisión del usuario, que se perdía entre demasiados bloques y texto). «Aplicar N cambios» publica una sola revisión (`apply_safe_corrections` con `imputeMissing`), así que un «Deshacer» la revierte entera. «Personalizar paso a paso» hace una pregunta por pantalla e incluye la normalización de nombres. El diagnóstico, el historial, las transformaciones y las acciones individuales siguen disponibles en «Diagnóstico y herramientas avanzadas». Medido con el dataset sintético de los E2E (1440 px, tema oscuro): 193 → 109 palabras visibles, 17 → 13 controles y 10 → 5 bloques; con datos reales habrá más señales. Maqueta aprobada: https://claude.ai/artifact/N3PuENPrgbLYTTn5hg9TQG. | `src/features/prepare/PrepareProposal.tsx`, `src/features/prepare/proposalModel.ts`, `src-tauri/src/dataset.rs` (`safe_corrected_plan_frame`) |
...
| 2026-09-21 | RV16: comparación de revisiones del historial y deltas de calidad extraídos a `dataset/snapshot_comparison.rs`. Las pruebas dedicadas pasan 3/3; `cargo check --tests` y `cargo test --lib` pasan, con 494 aprobadas, 5 ignoradas y ninguna fallida. La versión sigue en `1.25.0`; permanecen abiertos los gates externos de RV07/RV09/RV10/RV11. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/snapshot_comparison.rs`, `src-tauri/src/dataset/snapshot_comparison_tests.rs`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV16: el octavo corte mueve las correlaciones numéricas eager y source-backed, el muestreo y Pearson a `dataset/numeric_profile.rs`, preservando la API y los límites existentes. `cargo check --tests`, formato, documentación y la suite Rust completa pasan: 494 aprobadas, 0 fallidas y 5 ignoradas. La versión continúa en `1.25.0`; RV16 sigue abierta para extracciones graduales. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/numeric_profile.rs`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV16: el noveno corte extrae los contratos de reglas de calidad y `QualityValidationResult::total_invalid_count` a `dataset/quality_contracts.rs`; `dataset.rs` conserva las mismas rutas públicas mediante reexportaciones. Se mantienen privacidad de campos, nombres JSON y comportamiento. `cargo check --tests`, formato, documentación y la suite Rust completa pasan: 494 aprobadas, 0 fallidas y 5 ignoradas. La versión sigue en `1.25.0`. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/quality_contracts.rs`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV16: el décimo corte extrae la migración, validación y persistencia de documentos de reglas a `dataset/quality_documents.rs`, incluida la carga/salvado atómico y compatibilidad histórica. Los comandos conservan sus rutas y contratos; el parser de fechas compartido sigue disponible para el evaluador. `cargo check --tests` pasa y la suite Rust completa registra 494 aprobadas, 0 fallidas y 5 ignoradas. La versión continúa en `1.25.0`. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/quality_documents.rs`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-21 | RV16: el undécimo corte extrae la validación y evaluación eager/source-backed de reglas de calidad a `dataset/quality_evaluation.rs`, incluidos resultados, agregaciones, cancelación y preflight de exportación. Los helpers usados por el perfil temporal siguen accesibles dentro del motor; las rutas de comandos y contratos se conservan. `cargo check --tests` pasa y la suite Rust completa registra 494 aprobadas, 0 fallidas y 5 ignoradas. La versión sigue en `1.25.0`. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/quality_evaluation.rs`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md`
| 2026-09-21 | RV16: el duodécimo corte extrae validación y ejecución lazy de recetas a `dataset/recipe_engine.rs`; `dataset.rs` conserva el dispatcher y las rutas internas requeridas por source-backed. Se conservan los contratos y el fallback eager. `cargo fmt --check` y `cargo check --tests` pasan; no se ejecutó la suite completa tras este corte (la corrida previa registró 494 aprobadas y 5 ignoradas). La versión continúa en `1.25.0`. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/recipe_engine.rs`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md`
| 2026-09-21 | RV16: el decimotercer corte extrae la planificación y ejecución source-backed de recetas a `dataset/recipe_source_projection.rs`. Se preservan cancelación, materialización Parquet y helpers DuckDB compartidos. `cargo fmt --check`, `cargo check --tests` y la suite Rust completa pasan (494 aprobadas, 0 fallidas y 5 ignoradas). La versión sigue en `1.25.0`. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/recipe_source_projection.rs`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |

| 2026-09-22 | RV04: si `cancel_operation` o `discard_dataset_selection` falla al cancelar una selección previa a la importación, Columnia conserva su `selectionId` y marca cuál paso debe repetirse. «Reintentar cancelación» ejecuta solo el paso pendiente y mantiene bloqueada la inspección hasta completar; el dataset activo se conserva. `npm run build` pasa; no se ejecutaron pruebas de producto. Versión `1.26.0`; siguen abiertos la aceptación nativa/con datasets reales y los límites de APIs síncronas. | `src/App.tsx`, `src/features/load/LoadPhase.tsx`, `src/features/load/loadModel.ts`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |

| 2026-09-22 | RV16: se extraen `HistoryResult`, `HistoryEntryState` y `HistoryState` a `dataset/history_contracts.rs`; `dataset.rs` los reexporta para conservar las rutas existentes y los nombres JSON. `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check` y `cargo check --manifest-path src-tauri/Cargo.toml --lib` pasan; no se ejecutaron pruebas de producto. La versión sigue en `1.26.0`; RV16 permanece en curso. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/history_contracts.rs`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-22 | RV16: `HistoryManager` y sus contratos serializados se consolidan en `dataset/history.rs`; `dataset.rs` mantiene los reexports y rutas de uso existentes, incluidos los nombres JSON. `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y `git diff --check` pasan. No se ejecutaron pruebas de producto. La versión sigue en `1.26.0`; RV16 permanece en curso. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/history.rs`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
| 2026-09-22 | RV02/RV16: `preview_dataset_selection` delega el preflight del esquema en `dataset/import_schema_preview.rs` y conserva nombre, argumentos y ruta Tauri. Se mantienen la validación del perfil, cancelación y comprobación de que la fuente no cambió. Pasan formato Rust, compilación de la biblioteca, inventario IPC (85/4/70), validador documental estático y `git diff --check`; no se ejecutaron pruebas de producto. Versión `1.26.0`; RV02 sigue parcial a la espera de datasets reales y RV16 en curso. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/import_schema_preview.rs`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |


| 2026-09-22 | RV16: `get_dataset_page` delega en `dataset/page_reader.rs`; el wrapper conserva la firma/ruta Tauri y el lector mantiene generación de cancelación, snapshot Parquet, fallback source-backed y materialización final. Pasan formato Rust, `cargo check --lib`, IPC (85/4/70), checker documental y `git diff --check`; no se ejecutaron pruebas de producto en este corte. Versión `1.26.0`; RV16 queda en 20 módulos y 21 responsabilidades. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/page_reader.rs`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
...
| 2026-09-22 | RV16: `compare_dataset` se reúne con `get_dataset_conflict_page` en `dataset/comparison_reader.rs`; `dataset.rs` conserva los wrappers Tauri. Se mantienen la selección nativa, snapshots/fallbacks, cancelación, publicación atómica y comprobación de comparación vigente. `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` (58 advertencias de código inactivo, sin errores), IPC (85/4/70), checker documental y `git diff --check` pasan. No se ejecutaron pruebas de producto; versión `1.26.0`; RV16 queda en veintidós módulos y veinticinco responsabilidades. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/comparison_reader.rs`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
...
| 2026-09-22 | RV04/RV16: Deshacer/Rehacer comprueba cancelación en las restauraciones eager/source-backed, incluida la lectura Parquet por lotes, y antes de publicar el cursor. `compare_history_snapshots` cancela al cargar ambas revisiones por lotes; Resolver conflictos/Consolidar cancelan la lectura eager y la consulta local con Polars cancela la carga del snapshot comparado. Si cancelar gana, conserva la revisión activa y no entrega un resultado parcial. La vista previa de las cargas source-backed CSV/TSV/TXT/Parquet, JSON y Excel consulta cancelación por lote; `read_parquet_frame` queda exclusivo para tests y las lecturas productivas usan la variante cancelable. Pasan `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` (58 advertencias de código inactivo, sin errores), `npm run ipc:check` (85/4/70), `node tools/check-documentation.mjs` y `git diff --check`. No se ejecutaron pruebas de producto; versión `1.26.0`. | `src-tauri/src/dataset.rs`, `src-tauri/src/dataset/history.rs`, `src-tauri/src/dataset/snapshot_comparison.rs`, `ROADMAP.md`, `CONTEXTO.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |
...
| 2026-09-22 | RV02: `preview_dataset_selection` devuelve filas, columnas, tipos y diferencias con el perfil sin activar el candidato. La UI enseña el esquema antes de la confirmación final y solo aplica un perfil compatible; la carga final vuelve a leer el archivo. `npm run build`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` e `npm run ipc:check` pasan. No se ejecutaron pruebas de producto; falta aceptación con datasets de trabajo reales. Versión `1.25.0`; RV02 sigue parcial. | `src-tauri/src/dataset.rs`, `src-tauri/src/lib.rs`, `src/bridge/datasets.ts`, `src/features/load/LoadPhase.tsx`, `ROADMAP.md`, `docs/reference/roadmap-current.md`, `CHANGELOG.md` |

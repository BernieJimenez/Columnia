...
| --- | --- |
| Última actualización | 2026-09-22; RV04 conserva la selección pendiente y permite reintentar cancelación/liberación sin perder `selectionId`; RV02 revisa esquema candidato y discrepancias antes de activar, con aceptación en datasets reales pendiente; RV05 conserva evidencia de reinicio sintético; RV16 mantiene diecisiete módulos y reúne la implementación y los contratos serializados del historial en `dataset/history.rs`. `npm run build` pasó en RV04; `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y `git diff --check` pasan en RV16. No se ejecutaron pruebas de producto. Cola vigente en `docs/reference/roadmap-current.md` |
...
| Última revisión de este documento | 2026-09-22; documenta el reintento de cancelación/liberación de selecciones pendientes de RV04 y la consolidación de `HistoryManager` con sus contratos serializados en `dataset/history.rs`. `npm run build` pasa en RV04; `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check` y `cargo check --manifest-path src-tauri/Cargo.toml --lib` pasan en RV16. No se ejecutaron pruebas de producto. La versión vigente es `1.26.0`; RV04 sigue parcial y los gates abiertos están en `docs/reference/roadmap-current.md`. |

...
- 2026-08-30 P1 reduce el pico de los `JOIN` SQL locales `INNER`/`LEFT` sin agregación: el lado `dataset` se divide en bloques, cada bloque se une y consulta de forma cancelable, y solo se conserva la página global, el conteo y un bloque temporal. `FULL` y las agregaciones siguen usando la ruta completa dentro de los límites explícitos; DuckDB y la ejecución incremental general permanecen pendientes.
- 2026-08-30 P1 extiende esa ruta por bloques a las agregaciones `INNER`/`LEFT`: `COUNT`, `SUM`, `AVG`, `MIN` y `MAX` fusionan sus estados y grupos en el orden de primera aparición, sin conservar el `DataFrame` unido completo. `FULL`, DuckDB y la ejecución incremental general permanecen pendientes.

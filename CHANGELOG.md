...
- T10-20: `THREAT_MODEL.md` cubre ahora la entrega ODBC, el SQL local con
  DuckDB y los catálogos locales, con cifras actuales del inventario IPC.
...
- T10-08: el gate documental ya no acepta la versión mencionada en prosa:
  exige la cabecera `## [Unreleased]` o `## [x.y.z]`, y los perfiles Release y
  Package exigen la sección de la versión antes de publicar.
...
- RV16 extrae la coordinación de generaciones, locks de publicación y cancelación
  de `DatasetState` a `dataset/operation_state.rs`; las operaciones siguen
  rechazando resultados obsoletos y conservan sus contratos internos.
- RV02/RV04/RV16 separa el parseo cancelable de JSON/JSONL y la inspección de
  columnas en `dataset/json_reader.rs`; se conservan límites, callbacks,
...
- RV02/RV04/RV16 separa en `dataset/source_loading.rs` la carga source-backed, la materialización cancelable, los límites de recursos y los loaders por formato. Se conservan los contratos Tauri, los callbacks de progreso, la detección de cambios en la fuente y los errores de presupuesto; `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check` y `cargo check --manifest-path src-tauri/Cargo.toml --lib` pasan.
- RV02/RV04/RV16 separa en `dataset/spreadsheet_io.rs` la inspección de libros, la conversión tipada de rangos y la escritura/lectura de snapshots por bloques. Se conservan la selección de hojas, los modos de encabezado, los límites de recursos, la cancelación y los contratos Tauri; `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y la suite Rust (494/494 ejecutables) pasan.
- RV04/RV11/RV14/RV16 separa en `dataset/export_io.rs` los escritores y publicadores atómicos de CSV, JSON, Parquet, SQL, Excel, SQLite y Bundle, junto con privacidad y cancelación. Se conserva `copy_file_with_cancel` para proyectos, los contratos Tauri y la salida previa ante fallos; `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y la suite Rust (494/494 ejecutables) pasan.
- RV02/RV04/RV16 separa en `dataset/comparison_io.rs` la carga y persistencia temporal de fuentes comparadas, incluidos snapshots, CSV/JSON/Excel y cancelación. Se conservan los contratos de comparación, publicación atómica y errores; `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y la suite Rust (494/494 ejecutables) pasan.
- RV02/RV04/RV16 separa en `dataset/recipe_eager.rs` la validación, conversiones, filtros, resúmenes y aplicación de recetas sobre DataFrame eager. Se conservan excepciones, contratos y la selección lazy/source-backed; `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y la suite Rust (494/494 ejecutables) pasan.
- RV02/RV04/RV16 separa en `dataset/comparison_engine.rs` los índices derramados, la comparación por claves, los conflictos y JOIN con cancelación. Se conservan las rutas Tauri y los contratos de comparación; `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y la suite Rust (494/494 ejecutables) pasan.
...
- RV16 integra `get_dataset_page` en `dataset/page_reader.rs`; la ruta Tauri conserva su firma y delega la cancelación, snapshot y fallbacks en el lector correspondiente. `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y `npm run ipc:check` pasan. No se ejecutaron pruebas de producto.
- RV16 delega la orquestación de `get_dataset_profile` en `dataset/profile_reader.rs`; el wrapper conserva el comando Tauri y se mantienen cache, rutas eager/source-backed y cancelación. `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y `npm run ipc:check` pasan. No se ejecutaron pruebas de producto.
- RV16 delega `get_temporal_aggregation` en `dataset/profile_reader.rs`; conserva validación de columnas, snapshots/fallbacks source-backed y cancelación sin cambiar el comando Tauri. `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y `npm run ipc:check` pasan. No se ejecutaron pruebas de producto.
- RV16 delega `get_dataset_conflict_page` en `dataset/conflict_reader.rs`; conserva la ruta Tauri, cancelación, validación de comparación vigente y fallbacks. `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y `npm run ipc:check` pasan. No se ejecutaron pruebas de producto.
- RV16 reúne `compare_dataset` y `get_dataset_conflict_page` en `dataset/comparison_reader.rs`; conserva rutas Tauri, snapshots, cancelación y publicación atómica. `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo check --manifest-path src-tauri/Cargo.toml --lib` y `npm run ipc:check` pasan. No se ejecutaron pruebas de producto.

...
use history::{HistoryCommitResult, HistoryEntry, HistoryManager, PreparedHistorySnapshot};
...
use snapshot_comparison::compare_snapshot_frames;
...
struct SourceBackedConflictResolutionRequest {
    context: SourceBackedJoinContext,
    compared_path: PathBuf,
    compared_size_bytes: u64,
...
fn export_source_backed_csv_atomic<F, C>(
    source_path: &Path,
    expected_file_size: u64,
    destination: &Path,
    mut report: F,
...
struct SpilledKeyPayloadRows {
    _directory: tempfile::TempDir,
    bucket_paths: Vec<PathBuf>,
}

#[derive(Clone)]
struct KeyPayloadGroup {
    first_row_index: usize,
    count: usize,
    payload: Option<String>,
}

fn create_spilled_key_payload_rows() -> Result<SpilledKeyPayloadRows, String> {
    let directory = tempfile::tempdir().map_err(|error| {
...
fn common_row_count_between_parquet(
    current_path: &Path,
    current_row_count: usize,
    compared_path: &Path,
    compared_row_count: usize,
...
struct ParquetComparisonSource<'a> {
    path: &'a Path,
    row_count: usize,
}

fn compare_keyed_parquet_sources(
...
fn recipe_filter_uses_temporal_literal(operator: RecipeFilterOperator) -> bool {

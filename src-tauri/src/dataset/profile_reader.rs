...
pub(super) async fn get_dataset_profile_impl(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        // The analysis can take minutes on large files. It runs without holding
        // `current`, so history and page reads do not wait for it (T10-16);
        // the result is cached only if the dataset did not change meanwhile.
        let (work, stamp) = {
            ...
            if cached_source_is_valid {
                if let Some(profile) = &dataset.profile {
                    ...
                    let has_temporal_candidate = profile.columns.iter().any(|column| {
                        // Polars reports "date" and "datetime[μs]" (lower case).
                        let data_type = column.data_type.to_ascii_lowercase();
                        (data_type == "date" || data_type.starts_with("datetime"))
            ...
            let work = if dataset.source_backed {
        ...
        let profile = match work {
            ...
            ProfileWork::Frame(frame) => profile_dataset_with_progress(
                &frame,
                |stage, percent| send_progress(&on_progress, "profile", stage, percent),
                is_cancelled,
                correlation_sample_rows,
        ...
        let mut current = state.current.lock_recovering();
        if let Some(dataset) = current.as_mut().filter(|dataset| stamp.matches(dataset)) {

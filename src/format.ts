...
export function formatDataType(dataType: string): string {
  ...
  if (/^u?int(8|16|32|64|128)$|^(tiny|small|big|hug)?int(eger)?$/.test(normalized)) return "Entero";
  if (/^float(32|64)$|^(double|real|decimal.*)$/.test(normalized)) return "Decimal";
  if (normalized.startsWith("datetime") || normalized.startsWith("timestamp")) return "Fecha y hora";

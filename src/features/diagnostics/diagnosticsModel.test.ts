...
describe("contrato de diagnóstico local v1", () => {
  ...
  it("limita códigos y exige coherencia entre el estado y los códigos seleccionados", () => {
    expect(() => createDiagnosticReport({
      appVersion: "1.25.0", phase: "review", status: "issue_reported", errorCodes: [], metrics: null,
    })).toThrow();
    expect(() => createDiagnosticReport({
      appVersion: "1.25.0", phase: "review", status: "ready", errorCodes: ["DATASET_LOAD_FAILED"], metrics: null,
    })).toThrow();
    expect(() => createDiagnosticReport({
      appVersion: "1.25.0",
      phase: "review",

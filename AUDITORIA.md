...
| `npm run ipc:check` | Aprobado; 84 comandos de producción, 4 debug y 68 estructuras compartidas | El inventario se genera desde `generate_handler!` y se publica en [`ipc-inventory.json`](docs/reference/ipc-inventory.json); incluye tareas reutilizables, respaldo recuperable, preflight/presets de entrega y updater autenticado |
